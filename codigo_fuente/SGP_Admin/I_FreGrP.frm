...
Begin VB.Form I_FreGrP 
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.Frame Frame2 
         ...
         Width           =   4575
         Begin VB.OptionButton Option1 
            Caption         =   "Frec. Recetas Sin Costo"
            BeginProperty Font 
               Name            =   "MS Sans Serif"
               Size            =   8.25
               Charset         =   0
         ...
         End
         Begin VB.OptionButton Option1 
            Caption         =   "Frec. Recetas Con Costo"
            BeginProperty Font 
               Name            =   "MS Sans Serif"
               Size            =   8.25
               Charset         =   0
...
If vg_Indppr = 1 Or vg_Indppr = 2 Then
   ...
   Set RS = vg_db.Execute("SELECT * FROM a_regimen With(NoLock) WHERE reg_codigo=" & fpLongInteger1(1).Value & " and reg_indppr='" & vg_Indppr & "'")
    
Else
      
   Set RS = vg_db.Execute("SELECT * FROM a_regimen With(NoLock) WHERE reg_codigo=" & fpLongInteger1(1).Value & "")
...
Case 1
    ...
    If Option1(0).Value = True Or Option1(4).Value = True Then
...
Case 3
    ...
    Set RS = vg_db.Execute("sgpadm_Sel_CecoMinutaBloque '" & Trim(LimpiaDato(fpText.text)) & "'")
    If RS.EOF Then
...
Set RS = vg_db.Execute("sgpadm_Sel_ServicioMinutaBloqueMes '" & LimpiaDato(Trim(fpText.text)) & "', " & fpLongInteger1(1).Value & ", " & Format(fpDateTime1.text, "yyyymm") & "")
vaSpread1.MaxRows = 0
