...
Begin VB.Form T_EfectoMeteorizanteReceta 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Efecto Meteorizante Receta"
   ClientHeight    =   4950
   ClientLeft      =   2940
   ClientTop       =   1110
   ClientWidth     =   11715
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.ComboBox Combo1 
         Appearance      =   0  'Flat
         Height          =   315
         ItemData        =   "T_EfectoMeteorizanteReceta.frx":0000
         Left            =   2010
         List            =   "T_EfectoMeteorizanteReceta.frx":000A
   ...
   Begin FPSpread.vaSpread vaSpread1 
      ...
      MaxCols         =   6
      ProcessTab      =   -1  'True
      SpreadDesigner  =   "T_EfectoMeteorizanteReceta.frx":001E
      ScrollBarTrack  =   3
      ClipboardOptions=   0
   End
End
Attribute VB_Name = "T_EfectoMeteorizanteReceta"
...
If modo = "A" Then
   ...
   codigo = 0
   Set RS = vg_db.Execute("sgpadm_Ins_EfectoMeteorizanteReceta '" & Trim(Nombre) & "', '" & vg_NUsr & "'")
   If Not RS.EOF Then
...
Else
   ...
   Set RS = vg_db.Execute("sgpadm_Sel_CodigoEfectoMeteorizanteReceta " & codigo & "")
   If Not RS.EOF Then
       
      MensajeAnterior = codigo & ";" & IIf(IsNull(RS!NombreEfectoMeteorizante), "", Trim(RS!NombreEfectoMeteorizante)) & ";" & IIf(IsNull(RS!Activo), "", Trim(RS!Activo)) & ";" & vg_NUsr
   ...
   Set RS = vg_db.Execute("sgpadm_Upd_EfectoMeteorizanteReceta " & codigo & ", '" & Trim(Nombre) & "', '" & Activo & "', '" & vg_NUsr & "'")
...
Me.Height = 5385
Me.Width = 11820
MsgTitulo = "Efecto Meteorizante Receta"
...
If Combo1.ItemData(Combo1.ListIndex) = 0 Then
   ...
   Set RS = vg_db.Execute("sgpadm_Sel_BuscarCodigoEfectoMeteorizanteReceta '%" & UCase(LimpiaDato(fpText1.text)) & "%'")

ElseIf Combo1.ItemData(Combo1.ListIndex) = 1 Then
    
   Set RS = vg_db.Execute("sgpadm_Sel_BuscarNombreEfectoMeteorizanteReceta '%" & UCase(LimpiaDato(fpText1.text)) & "%'")
...
If Not RS.EOF Then
   ...
   Do While Not RS.EOF
      ...
      vaSpread1.Value = RS!IdEfectoMeteorizante
      
      vaSpread1.Col = 2
      vaSpread1.Value = IIf(IsNull(RS!NombreEfectoMeteorizante), "", Trim(RS!NombreEfectoMeteorizante))
...
Case 5
    ...
    Set RS = vg_db.Execute("sgpadm_Del_EfectoMeteorizanteReceta " & codigo & ", '0', '" & vg_NUsr & "'")
...
Set RS = vg_db.Execute("sgpadm_Sel_EfectoMeteorizanteReceta")
