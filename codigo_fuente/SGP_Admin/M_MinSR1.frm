...
Set RS = vg_db.Execute("sgpadm_Sel_ListarMinutaBloquexCeco '" & cencos & "', " & Val(fpLongInteger1(0).Value) & ", " & Val(fpLongInteger1(1).Value) & "")
...
Dim Año     As String
    ...
    If DateDiff("d", FpFecDesde.text, FpFecHasta.text) > 98 Then
...
On Error GoTo Man_Error
    ...
    swEsMinBloque = False 'MVA - MVI - VAR. PARA SABER SI ENTRÓ AL FORMULARIO.
...
Col = IIf(Index = 0, 2, IIf(Index = 1, 3, IIf(Index = 2, 4, IIf(Index = 3, 5, 6))))
vaSpread1.Visible = False
    ...
    Select Case Button.Index
        ...
        Case 1, 3
            ...
            If Button.Index = 1 Then
               ...
               Set RS = vg_db.Execute("sgpadm_Sel_ValidarMinutaBloqueNuevo " & Sql & "")
               If Not RS.EOF Then
...
'               Set RS = vg_db.Execute("select isnull(cli_tipominuta,0) as cli_tipominuta from b_clientes with (nolock) where cli_codigo = '" & LimpiaDato(Trim(fpText.text)) & "' and cli_tipo = 0 and cli_tipominuta = 1")
               Set RS = vg_db.Execute("select isnull(cli_tipominuta,0) as cli_tipominuta from b_clientes with (nolock) where cli_codigo = '" & LimpiaDato(Trim(fpText.text)) & "' and cli_tipo = 0 and cli_tipominuta = 4")
               If Not RS.EOF Then
                  
                  RS.Close
                  Set RS = Nothing
'                  MsgBox "Ceco tiene asignado tipo de minuta BLOQUE no puede realizar una minuta, esto empieza desde el sitio", vbExclamation + vbOKOnly, Me.Caption
                  MsgBox "Ceco tiene asignado tipo de minuta OTROS no puede realizar una minuta, esto empieza desde el sitio", vbExclamation + vbOKOnly, Me.Caption
                  Exit Sub
            ...
            ElseIf Button.Index = 3 Then
               ...
               Set RS = vg_db.Execute("sgpadm_Sel_ValidarMinutaBloqueModificado " & Sql & "")
               If RS.EOF Then
            ...
            Set RS = vg_db.Execute("sgpadm_s_ValidaDatos_V02 1, '" & LimpiaDato(Trim(fpText.text)) & "',0,0,'',0,'',0,0,0,0,''")
            If RS.EOF Then
...
'Tipo 4
                ...
                Set RS = vg_db.Execute("sgpadm_s_ValidaDatos_V02 4,'',0,0,'" & LimpiaDato(Trim(fpText.text)) & "'," & Val(fpLongInteger1(2).Value) & ",'',0,0,0,0,''")
                If RS.EOF Then
...
'            End If
            ...
            Set RS = vg_db.Execute("sgpadm_Sel_TraerConceptoClienteAMD_V01 '" & LimpiaDato(Trim(vg_codcasino)) & "'")
            If Not RS.EOF Then
                
                
                If RS!cli_ADM = "2" Then
                
                   vg_opcionmenubloque = RS!cli_ADM
                ...
                ElseIf RS!cli_ADM = "1" Then
                   ...
                   Unload M_MinBloqueADM2
            
                   Call M_MinBloqueADM2.Show(1)
...
'            DropTebleTmp (LimpiaDato(Trim(fpText.text)) & Val(fpLongInteger1(0).Value) & Val(fpLongInteger1(1).Value) & Format(FpFecDesde.text, "yyyymmdd") & Format(FpFecHasta.text, "yyyymmdd"))
        
        Case 6 'Eliminar registro minuta bloque
            ...
            For i = 1 To vaSpread1.MaxRows
                ...
                If vaSpread1.text = "1" Then
                   ...
                   Set RS = vg_db.Execute("sgpadm_Del_MinutaBloque_V02 " & Sql & "")
