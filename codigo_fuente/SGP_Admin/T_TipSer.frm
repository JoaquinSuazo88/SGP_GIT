...
If modo = "A" Then
   ...
   codigo = 0
   Set RS = vg_db.Execute("sgpadm_iu_tiposervicio 'A', 0, '" & Trim(Nombre) & "'")
   If Not RS.EOF Then
...
Else
   ...
   vg_db.Execute "sgpadm_iu_tiposervicio 'M', " & codigo & ", '" & Trim(Nombre) & "'"
...
If Combo1.ItemData(Combo1.ListIndex) = 0 Then
   ...
   Set RS = vg_db.Execute("sgpadm_s_tiposervicio 3, 0, '%" & UCase(LimpiaDato(fpText1.text)) & "%'")

ElseIf Combo1.ItemData(Combo1.ListIndex) = 1 Then
    
   Set RS = vg_db.Execute("sgpadm_s_tiposervicio 4, 0, '%" & UCase(LimpiaDato(fpText1.text)) & "%'")
...
If Not RS.EOF Then
   ...
   Do While Not RS.EOF
      ...
      vaSpread1.Value = RS!tis_codigo
      
      vaSpread1.Col = 2
      vaSpread1.Value = IIf(IsNull(RS!tis_nombre), "", Trim(RS!tis_nombre))
...
Set RS = vg_db.Execute("SELECT * FROM a_tiposervicio ORDER BY tis_codigo")
...
Set RS = vg_db.Execute("SELECT * FROM a_tiposervicio WHERE tis_codigo=" & codigo)
If Not RS.EOF Then
