...
Begin VB.Form T_IngCruceGarnituraReceta 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Ingrediente Cruce Garnitura Receta"
   ClientHeight    =   4950
   ClientLeft      =   2940
   ClientTop       =   1110
   ClientWidth     =   11760
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.ComboBox Combo1 
         Appearance      =   0  'Flat
         Height          =   315
         ItemData        =   "T_IngCruceGarnituraReceta.frx":0000
         Left            =   2010
         List            =   "T_IngCruceGarnituraReceta.frx":000A
   ...
   Begin FPSpread.vaSpread vaSpread1 
      ...
      MaxCols         =   6
      ProcessTab      =   -1  'True
      SpreadDesigner  =   "T_IngCruceGarnituraReceta.frx":001E
      ScrollBarTrack  =   3
      ClipboardOptions=   0
   End
End
Attribute VB_Name = "T_IngCruceGarnituraReceta"
...
If modo = "A" Then
   ...
   codigo = 0
   Set RS = vg_db.Execute("sgpadm_Ins_IngCruceGarnituraReceta '" & Trim(Nombre) & "', '" & vg_NUsr & "'")
   If Not RS.EOF Then
...
Else
   ...
   Set RS = vg_db.Execute("sgpadm_Sel_CodigoIngCruceGarnituraReceta " & codigo & "")
   If Not RS.EOF Then
       
      MensajeAnterior = codigo & ";" & IIf(IsNull(RS!NombreIngCruceGarnitura), "", Trim(RS!NombreIngCruceGarnitura)) & ";" & IIf(IsNull(RS!Activo), "", Trim(RS!Activo)) & ";" & vg_NUsr
   ...
   Set RS = vg_db.Execute("sgpadm_Upd_IngCruceGarnituraReceta " & codigo & ", '" & Trim(Nombre) & "', '" & Activo & "', '" & vg_NUsr & "'")
...
Me.Height = 5385
Me.Width = 11820
MsgTitulo = "Ingrediente Cruce Garnutura Receta"
...
If Combo1.ItemData(Combo1.ListIndex) = 0 Then
   ...
   Set RS = vg_db.Execute("sgpadm_Sel_BuscarCodigoIngCruceGarnituraReceta '%" & UCase(LimpiaDato(fpText1.text)) & "%'")

ElseIf Combo1.ItemData(Combo1.ListIndex) = 1 Then
    
   Set RS = vg_db.Execute("sgpadm_Sel_BuscarNombreIngCruceGarnituraReceta '%" & UCase(LimpiaDato(fpText1.text)) & "%'")
...
If Not RS.EOF Then
   ...
   Do While Not RS.EOF
      ...
      vaSpread1.Value = RS!IdIngCruceGarnitura
      
      vaSpread1.Col = 2
      vaSpread1.Value = IIf(IsNull(RS!NombreIngCruceGarnitura), "", Trim(RS!NombreIngCruceGarnitura))
...
Case 5
    ...
    Set RS = vg_db.Execute("sgpadm_Del_IngCruceGarnituraReceta " & codigo & ", '0', '" & vg_NUsr & "'")
...
Set RS = vg_db.Execute("sgpadm_Sel_IngCruceGarnituraReceta")
