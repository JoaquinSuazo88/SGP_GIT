...
Begin VB.Form T_Segmen 
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.ComboBox Combo1 
         Appearance      =   0  'Flat
         Height          =   315
         ItemData        =   "T_Segme.frx":0000
         Left            =   2010
         List            =   "T_Segme.frx":000A
   ...
   Begin FPSpread.vaSpread vaSpread1 
      ...
      MaxCols         =   2
      ProcessTab      =   -1  'True
      SpreadDesigner  =   "T_Segme.frx":001E
      ScrollBarTrack  =   3
      ClipboardOptions=   0
   End
...
If modo = "A" Then
    RS1.Open "SELECT seg_codigo FROM a_segmento ORDER BY seg_codigo DESC", vg_db, adOpenStatic
    If Not RS1.EOF Then
        RS1.MoveFirst
        codigo = RS1!seg_codigo + 1
    Else
        codigo = 1
    End If
    RS1.Close: Set RS1 = Nothing
    vg_db.BeginTrans
    vg_db.Execute "INSERT INTO a_segmento (seg_codigo, seg_nombre) VALUES (" & codigo & ", '" & Trim(Nombre) & "')"
    vg_db.CommitTrans
    vaSpread1.Col = 1: vaSpread1.Value = codigo
Else
    vg_db.BeginTrans
    vg_db.Execute "UPDATE a_segmento SET seg_nombre='" & Trim(Nombre) & "' WHERE seg_codigo=" & codigo
    vg_db.CommitTrans
End If
...
If Combo1.ItemData(Combo1.ListIndex) = 0 Then
    RS2.Open "SELECT seg_codigo,seg_nombre FROM a_segmento WHERE seg_codigo LIKE '%" & UCase(LimpiaDato(fpText1.text)) & "%'", vg_db, adOpenStatic
ElseIf Combo1.ItemData(Combo1.ListIndex) = 1 Then
    RS2.Open "SELECT seg_codigo, seg_nombre FROM a_segmento WHERE UPPER(seg_nombre) LIKE '%" & UCase(LimpiaDato(fpText1.text)) & "%'", vg_db, adOpenStatic
End If
...
If Not RS2.EOF Then
   Do While Not RS2.EOF
      ...
      vaSpread1.Value = RS2!seg_codigo
      vaSpread1.Col = 2
      vaSpread1.Value = IIf(IsNull(RS2!seg_nombre), "", Trim(RS2!seg_nombre))
      RS2.MoveNext
   Loop
   Gl_Ac_Botones Me, 1, 1, modo
End If
...
Case 5
    ...
    vaSpread1.Col = 1: codigo = Val(vaSpread1.Value)
    vg_db.BeginTrans
    vg_db.Execute "DELETE a_segmento FROM a_segmento WHERE seg_codigo=" & codigo
    vg_db.CommitTrans
...
vaSpread1.Visible = False
vaSpread1.MaxRows = 0
RS2.Open "SELECT * FROM a_segmento ORDER BY seg_codigo", vg_db, adOpenStatic
Do While Not RS2.EOF
    vaSpread1.MaxRows = vaSpread1.MaxRows + 1
    vaSpread1.Row = vaSpread1.MaxRows
    vaSpread1.Col = 1: vaSpread1.Value = RS2!seg_codigo
    vaSpread1.Col = 2: vaSpread1.Value = IIf(IsNull(RS2!seg_nombre), "", Trim(RS2!seg_nombre))
    RS2.MoveNext
Loop
RS2.Close: Set RS2 = Nothing
Gl_Ac_Botones Me, 1, 1, modo
...
vaSpread1.Row = vaSpread1.ActiveRow
vaSpread1.Col = 1: codigo = Val(vaSpread1.Value)
RS1.Open "SELECT * FROM a_segmento WHERE seg_codigo=" & codigo, vg_db, adOpenStatic
If Not RS1.EOF Then
   vaSpread1.Col = 2: vaSpread1.Value = IIf(IsNull(RS1!seg_nombre), "", Trim(RS1!seg_nombre))
End If
RS1.Close: Set RS1 = Nothing
OpGr = False
End Sub
