...
If modo = "A" Then
   ...
   codigo = 0
   Set RS = vg_db.Execute("sgpadm_iu_tipoactividad 'A', 0, '" & Trim(Nombre) & "'")
...
If Combo1.ItemData(Combo1.ListIndex) = 0 Then
    ...
    Set RS = vg_db.Execute("SELECT tia_codigo, tia_nombre FROM a_tipoactividad WHERE tia_codigo LIKE '%" & UCase(LimpiaDato(fpText1.text)) & "%'")

ElseIf Combo1.ItemData(Combo1.ListIndex) = 1 Then
    
    Set RS = vg_db.Execute("SELECT tia_codigo, tia_nombre FROM a_tipoactividad WHERE UPPER(tia_nombre) LIKE '%" & UCase(LimpiaDato(fpText1.text)) & "%'")
...
If Not RS.EOF Then
   ...
   Do While Not RS.EOF
      ...
      vaSpread1.text = RS!tia_codigo
      
      vaSpread1.Col = 2
      vaSpread1.Lock = False
      vaSpread1.TypeHAlign = TypeHAlignLeft
      vaSpread1.text = IIf(IsNull(RS!tia_nombre), "", Trim(RS!tia_nombre))
...
Set RS = vg_db.Execute("SELECT tia_codigo, tia_nombre FROM a_tipoactividad ORDER BY tia_codigo")
Do While Not RS.EOF
    ...
    vaSpread1.Col = 1
    vaSpread1.Lock = True
    vaSpread1.text = IIf(IsNull(RS!tia_codigo), "", RS!tia_codigo)
...
Set RS = vg_db.Execute("SELECT * FROM a_tipoactividad WHERE tia_codigo=" & codigo & "")
If Not RS.EOF Then
   
   vaSpread1.Col = 2
   vaSpread1.Value = IIf(IsNull(RS!tia_nombre), "", Trim(RS!tia_nombre))
