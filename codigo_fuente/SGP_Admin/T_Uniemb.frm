...
Begin VB.Form T_Uniemb 
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.ComboBox Combo1 
         Height          =   315
         ItemData        =   "T_Uniemb.frx":188F
         Left            =   1875
         List            =   "T_Uniemb.frx":1899
...
vaSpread1.Col = 2
Nombre = Trim(LimpiaDato(Mid(vaSpread1.Value, 1, 10)))
...
If modo = "A" Then
   ...
   indice = 0
   Set RS = vg_db.Execute("sgpadm_iu_uniemb 'A', 0, '" & Trim(Nombre) & "', '" & Trim(NomCor) & "'")
...
Else
   ...
   vg_db.Execute "sgpadm_iu_uniemb 'M', " & codigo & ", '" & Trim(Nombre) & "', '" & Trim(NomCor) & "'"
...
If Combo1.ItemData(Combo1.ListIndex) = 0 Then
   ...
   RS.Open "SELECT emb_codigo,emb_nombre,emb_nomcor FROM a_embalaje WHERE emb_codigo LIKE '%" & UCase(LimpiaDato(fpText1.text)) & "%'", vg_db, adOpenStatic

ElseIf Combo1.ItemData(Combo1.ListIndex) = 1 Then
    
   RS.Open "SELECT emb_codigo, emb_nombre, emb_nomcor FROM a_embalaje WHERE UPPER(emb_nombre) LIKE '%" & UCase(LimpiaDato(fpText1.text)) & "%'", vg_db, adOpenStatic
...
If Not RS.EOF Then
   ...
   Do While Not RS.EOF
      ...
      vaSpread1.Value = RS!emb_codigo
      
      vaSpread1.Col = 2
      vaSpread1.Value = Trim(RS!emb_nombre)
      
      vaSpread1.Col = 3
      vaSpread1.Value = Trim(RS!emb_nomcor)
...
Case 5
    ...
    vaSpread1.Col = 1
    codigo = Val(vaSpread1.Value)
    vg_db.Execute "DELETE FROM a_embalaje WHERE emb_codigo=" & codigo
...
vaSpread1.Visible = False
vaSpread1.MaxRows = 0
Set RS = vg_db.Execute("SELECT * FROM a_embalaje ORDER BY emb_codigo")
...
Set RS = vg_db.Execute("SELECT * FROM a_embalaje WHERE emb_codigo = " & codigo & "")
If Not RS.EOF Then
