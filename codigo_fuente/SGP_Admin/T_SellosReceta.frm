...
Begin VB.Form T_SellosReceta 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Sellos Receta"
   ClientHeight    =   4950
   ClientLeft      =   2940
   ClientTop       =   1110
   ClientWidth     =   11715
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.ComboBox Combo1 
         Appearance      =   0  'Flat
         Height          =   315
         ItemData        =   "T_SellosReceta.frx":0000
         Left            =   2010
         List            =   "T_SellosReceta.frx":000A
   ...
   Begin FPSpread.vaSpread vaSpread1 
      ...
      MaxCols         =   6
      ProcessTab      =   -1  'True
      SpreadDesigner  =   "T_SellosReceta.frx":001E
      ScrollBarTrack  =   3
      ClipboardOptions=   0
   End
End
Attribute VB_Name = "T_SellosReceta"
...
If modo = "A" Then
   ...
   codigo = 0
   Set RS = vg_db.Execute("sgpadm_Ins_SellosReceta '" & Trim(Nombre) & "', '" & vg_NUsr & "'")
   If Not RS.EOF Then
...
Else
   ...
   Set RS = vg_db.Execute("sgpadm_Sel_CodigoSellosReceta " & codigo & "")
   If Not RS.EOF Then
       
      MensajeAnterior = codigo & ";" & IIf(IsNull(RS!NombreSellos), "", Trim(RS!NombreSellos)) & ";" & IIf(IsNull(RS!Activo), "", Trim(RS!Activo)) & ";" & vg_NUsr
   ...
   Set RS = vg_db.Execute("sgpadm_Upd_SellosReceta " & codigo & ", '" & Trim(Nombre) & "', '" & Activo & "', '" & vg_NUsr & "'")
...
If Combo1.ItemData(Combo1.ListIndex) = 0 Then
   ...
   Set RS = vg_db.Execute("sgpadm_Sel_BuscarCodigoSellosReceta '%" & UCase(LimpiaDato(fpText1.text)) & "%'")

ElseIf Combo1.ItemData(Combo1.ListIndex) = 1 Then
    
   Set RS = vg_db.Execute("sgpadm_Sel_BuscarNombreSellosReceta '%" & UCase(LimpiaDato(fpText1.text)) & "%'")
...
If Not RS.EOF Then
   ...
   Do While Not RS.EOF
      ...
      vaSpread1.Value = RS!IdSellos
      
      vaSpread1.Col = 2
      vaSpread1.Value = IIf(IsNull(RS!NombreSellos), "", Trim(RS!NombreSellos))
...
Case 5
    ...
    Set RS = vg_db.Execute("sgpadm_Del_SellosReceta " & codigo & ", '0', '" & vg_NUsr & "'")
...
Set RS = vg_db.Execute("sgpadm_Sel_SellosReceta")
