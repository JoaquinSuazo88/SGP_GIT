...
Begin VB.Form M_SacSgp 
   ...
   Begin VB.Frame Frame1 
      ...
      Begin FPSpread.vaSpread vaSpread2 
         ...
         MaxCols         =   1
         MaxRows         =   1
         SpreadDesigner  =   "M_SacSgp.frx":0680
      End
...
Dim ModAgr    As String

Private Sub Combo1_Click(Index As Integer)

If lc_Aux = "SacSgp" Then
   
   codigo = Trim(fg_codigocbo(Combo1, 0, 10, ""))

Else
   
   codigo = Trim(fg_codigocbo(Combo1, 0, 9, ""))
...
Me.Caption = IIf(lc_Aux = "SacSgp", "Asociar Productos SAC vs SGP", "Asociar Productos SAP vs SGP")
Label1.Caption = IIf(lc_Aux = "SacSgp", "Familia productos SAC", "Familia productos SAP")
fg_centra Me
MsgTitulo = IIf(lc_Aux = "SacSgp", "Asociar Productos SAC vs SGP", "Asociar Productos SAP vs SGP")
modo = "": Est = True: codigo = ""
vg_auxcod = IIf(lc_Aux = "SacSgp", "sac", "sap")
Gl_Mo_Botones Me, 12
Combo1(0).Clear
...
Else
   ...
   Set RS = vg_db.Execute("sgpadm_s_catproductosap 1, '', ''")
...
Do While Not RS.EOF
    ...
    Combo1(0).AddItem IIf(IsNull(RS(1)), "", RS(1)) & Space(150) & IIf(lc_Aux = "SacSgp", "(" & fg_pone_cero(Str(RS(0)), 10) & ")", "(" & fg_pone_espacio((RS(0)), 9) & ")")
    RS.MoveNext
...
vaSpread1.Row = 0
vaSpread1.Col = 1
vaSpread1.text = IIf(lc_Aux = "SacSgp", "Categoria Productos SAC", "Categoria Productos SAP")
vaSpread1.Col = 3
vaSpread1.text = IIf(lc_Aux = "SacSgp", "Código              SAC", "Código Material     SAP")
vaSpread1.Col = 7
vaSpread1.text = IIf(lc_Aux = "SacSgp", "Cta. Con. (SAC)", "Cta. Con. (SAP)")
vaSpread1.MaxRows = 0
IndSac = 1
...
If lc_Aux = "SacSgp" Then
   ...
   vaSpread1.Col = 3
   vaSpread1.ColWidth(3) = 16.13
   vaSpread1.Col = 4
   vaSpread1.ColWidth(4) = 41.13
...
Else
   ...
   Set RS = vg_db.Execute("sgpadm_s_formatocomprassap 1, '" & codigo & "', ''")
...
ModAgr = IIf(Mid(ValidarUsuario(Me), 1, 1) = "1", 1, 0)
vaSpread1.Lock = IIf(ModAgr = "1", False, True)
vaSpread1.BackColor = Shape1(0).FillColor

Toolbar1.Buttons(1).Enabled = IIf(ModAgr, True, False)
Toolbar1.Buttons(17).ButtonMenus(1).Enabled = IIf(ModAgr, True, False)
Toolbar1.Buttons(5).Enabled = IIf(Mid(ValidarUsuario(Me), 1, 3) = "1", True, False)
...
'       vaSpread1.MaxRows = vaSpread1.MaxRows + 1
       vaSpread1.Row = IndSac 'vaSpread1.MaxRows
       
       vaSpread1.Col = 1
       vaSpread1.TypeHAlign = TypeHAlignLeft
       vaSpread1.text = IIf(IsNull(RS(4)), "", Trim(RS(4))) 'IIf(IsNull(RS!cap_nombre), "", Trim(RS!cap_nombre))
       ...
       vaSpread1.Col = 3
       vaSpread1.TypeHAlign = TypeHAlignLeft
       vaSpread1.text = IIf(IsNull(RS(0)), "", Trim(RS(0))) 'IIf(IsNull(RS!foc_codsac), "", Trim(RS!foc_codsac))
                
       vaSpread1.Col = 4
       vaSpread1.TypeHAlign = TypeHAlignLeft
       vaSpread1.text = IIf(IsNull(RS(1)), "", Trim(RS(1))) 'IIf(IsNull(RS!foc_nomsac), "", Trim(RS!foc_nomsac))
                
       vaSpread1.Col = 5
       vaSpread1.TypeHAlign = TypeHAlignLeft
       vaSpread1.text = IIf(IsNull(RS(2)), "", Trim(RS(2))) 'IIf(IsNull(RS!foc_unisac), "", Trim(RS!foc_unisac))
       
       vaSpread1.Col = 6
       vaSpread1.TypeHAlign = TypeHAlignLeft
       vaSpread1.text = IIf(IsNull(RS(3)), "", Format(fg_Ctod1(RS(3)), "dd/mm/yyyy")) 'IIf(IsNull(RS!foc_vigfin), "", Format(RS!foc_vigfin, "dd/mm/yyyy"))
                
       If lc_Aux <> "SacSgp" Then
          
          vaSpread1.Col = 7
          vaSpread1.TypeHAlign = TypeHAlignLeft
          vaSpread1.text = IIf(IsNull(RS(5)), "", Trim(RS(5)))
...
Case 1 '------> Incluir formato sgp
    ...
    vaSpread1.Row = Row
    vaSpread1.Col = 8: vaSpread1.text = Trim(vg_codigo)
    vaSpread1.Col = 9: vaSpread1.text = Trim(vg_nombre)
    vaSpread1.Col = 10: vaSpread1.text = Trim(vg_ames)
    vg_auxcod = IIf(lc_Aux = "SacSgp", "sac", "sap")
    Set RS = vg_db.Execute("sgpadm_Sel_productos 26, '" & Trim(LimpiaDato(vg_codigo)) & "', '" & vg_auxcod & "', '" & vg_NUsr & "'")
    If Not RS.EOF Then
...
Case 12 '-------> Grabar datos
    ...
    For i = 1 To vaSpread1.MaxRows
        ...
        vaSpread1.Col = 8: codsgp = "": codsgp = Trim(vaSpread1.text)
        
        If codsgp <> "" Then
           
           estvin = True
           
           If lc_Aux = "SacSgp" Then 'formato compras sac
              
              Set RS = vg_db.Execute("sgpadm_Sel_productos 27, '" & codsgp & "', '" & codsac & "', '" & vg_NUsr & "'")
              If RS.EOF Then
            ...
            ElseIf lc_Aux = "SapSgp" Then 'formato compras sap
              
              Set RS = vg_db.Execute("sgpadm_s_formatocomprassap 3, '" & codsgp & "', '" & codsac & "'")
              If RS.EOF Then
                 
                 vg_db.Execute "INSERT INTO b_formatocompras_sap_sgp (fss_CodMaterial, fss_CodSgp, fss_SgpPre) VALUES ('" & codsac & "', '" & codsgp & "', " & sgppre & ")"
    ...
    If estvin And lc_Aux = "SacSgp" Then
       
       vg_db.Execute "sgpadm_p_actuvinculofcompras"
    
    ElseIf estvin And lc_Aux = "SapSgp" Then
       
       vg_db.Execute "sgpadm_p_actuvinculofcomprassap"
...
Case 15 '-------> Fijar Vinculo
    ...
    vg_auxcod = IIf(lc_Aux = "SacSgp", "sac", "sap")
    Set RS = vg_db.Execute("sgpadm_Sel_productos 26, '" & Trim(LimpiaDato(vaSpread1.text)) & "', '" & vg_auxcod & "', '" & vg_NUsr & "'")
    If Not RS.EOF Then
    ...
    ElseIf lc_Aux = "SapSgp" Then
       
       Set RS = vg_db.Execute("sgpadm_s_formatocomprassap 2, '" & Trim(LimpiaDato(vaSpread1.text)) & "', ''")
       If Not RS.EOF Then MsgBox "Formato de compras SAP, ya esta vinculado" & VgLinea & VgLinea & Trim(RS!fcs_CodMaterial) & VgLinea & Trim(RS!fcs_DenMaterial), vbInformation + vbOKOnly, MsgTitulo: RS.Close: Set RS = Nothing: Exit Sub
       RS.Close: Set RS = Nothing
    ...
    For i = 1 To vaSpread1.MaxRows
        ...
        vaSpread1.Row = i
        vaSpread1.Col = 8: auxsgp = Trim(vaSpread1.text)
        vaSpread1.Col = 1
...
Case 19 '-------> Imprimir Productos no homologados casino
    
    I_ProSAPNoHol.Show 0

Case 22 '-------> Cerrar opción
...
Case "Desde Formato SAP"

       P_ImpTxt.llena_datos "1"
       P_ImpTxt.Show 1
       
       MoverDatosGrilla

Case "Desde Formato SAP Justicia"

       P_ImpTxt.llena_datos "2"
       P_ImpTxt.Show 1
...
If vaSpread1.MaxRows < 1 Or ModAgr = "0" Then Exit Sub
...
Case 3
    ...
    vaSpread1.Col = Col
    TipText = IIf(lc_Aux = "SacSgp", "Código Sac: ", "Código Sap: ") & vaSpread1.text
...
Case 5
    ...
    vaSpread1.Col = Col
    TipText = IIf(lc_Aux = "SacAgp", "Unidad Medida SAC : ", "Unidad Medida SAP : ") & Trim(vaSpread1.text)
