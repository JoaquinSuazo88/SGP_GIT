...
Begin VB.Form T_Unimed 
   ...
   Begin FPSpread.vaSpread vaSpread1 
      ...
      EndProperty
      MaxCols         =   3
      SpreadDesigner  =   "T_Unimed.frx":0000
      ScrollBarTrack  =   3
   End
   Begin VB.Frame Frame1 
      ...
      Begin VB.ComboBox Combo1 
         Height          =   315
         ItemData        =   "T_Unimed.frx":188F
         Left            =   2085
         List            =   "T_Unimed.frx":1899
...
If Trim(Nombre) = "" Or Trim(NomCor) = "" Then MsgBox "Falta información...", vbExclamation + vbOKOnly, MsgTitulo: vaSpread1.Row = Fila: vaSpread1.Col = 2: vaSpread1.SetActiveCell 2, vaSpread1.Row: vaSpread1.SetFocus: OpGr = False: Exit Sub
...
If modo = "A" Then
   ...
   indice = 0
   Set RS = vg_db.Execute("sgpadm_iu_unidadmed 'A', 0, '" & Trim(Nombre) & "', '" & Trim(NomCor) & "'")
   If Not RS.EOF Then
...
Else
   ...
   vg_db.Execute "sgpadm_iu_unidadmed 'M', " & codigo & ", '" & Trim(Nombre) & "', '" & Trim(NomCor) & "'"
...
If Combo1.ItemData(Combo1.ListIndex) = 0 Then
   ...
   RS.Open "SELECT unm_codigo,unm_nombre,unm_nomcor FROM a_unidadmed WHERE unm_codigo LIKE '%" & UCase(LimpiaDato(fpText1.text)) & "%'", vg_db, adOpenStatic

ElseIf Combo1.ItemData(Combo1.ListIndex) = 1 Then
    
   RS.Open "SELECT unm_codigo, unm_nombre, unm_nomcor FROM a_unidadmed WHERE UPPER(unm_nombre) LIKE '%" & UCase(LimpiaDato(fpText1.text)) & "%'", vg_db, adOpenStatic
...
If Not RS.EOF Then
   ...
   Do While Not RS.EOF
      ...
      vaSpread1.text = RS!unm_codigo
      
      vaSpread1.Col = 2
      vaSpread1.text = Trim(RS!unm_nombre)
...
Case 5
    ...
    vaSpread1.Col = 3
    NomCor = Trim(LimpiaDato(vaSpread1.text))
    
    vg_db.Execute "DELETE a_unidadmed FROM a_unidadmed WHERE unm_codigo=" & codigo
...
Do While Not RS.EOF
    ...
    vaSpread1.Col = 2
    vaSpread1.text = IIf(IsNull(RS!unm_nombre), "", Trim(RS!unm_nombre))
    
    vaSpread1.Col = 3
    vaSpread1.text = IIf(IsNull(RS!unm_nomcor), "", Trim(RS!unm_nomcor))
...
Set RS = vg_db.Execute("SELECT * FROM a_unidadmed WHERE unm_codigo=" & codigo)
If Not RS.EOF Then
