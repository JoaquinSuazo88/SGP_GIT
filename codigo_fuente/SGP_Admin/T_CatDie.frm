...
Set RS = vg_db.Execute("sgpadm_Sel_CategoriaDietetica_V02 4, 0, ''")
If Not RS.EOF Then
   
   Do While Not RS.EOF
      
      Set rootNode = TvwDir.Nodes.Add(nd, tvwChild, "H" & RS!car_codigo, Trim(RS!car_nombre))
      
      If RS!car_activo = "0" Then
...
If modo = "A" Or modo = "M" Then
   ...
   If modo = "A" Then
      '------- Crear arbol de receta tecfood
      indice = 0
      
      If RS.State = 1 Then RS.Close
      RS.CursorLocation = adUseClient
      vg_db.CursorLocation = adUseClient
      Set RS = vg_db.Execute("sgpadm_iu_recetacatdie_V02 'A', 0, '" & Mid(LimpiaDato(Trim(TvwDir.Nodes(dest.Index).text)), 1, 50) & "', " & codpadre & ", '" & vg_NUsr & "'")
      If Not RS.EOF Then
      ...
      If indice > 0 Then
         ...
         nivpro = "1": arvpro = "": inipro = 1: finpro = 1
         ...
         If codpadre <> 0 Then
            ...
            Set RS = vg_db.Execute("sgpadm_Sel_CategoriaDietetica_V02 2," & codpadre & ",''")
            If Not RS.EOF Then arvpro = CStr(RS!car_codigo): nivpro = IIf(RS!car_previo = 0, "2", "3"): inipro = IIf(RS!car_previo = 0, 2, 4)
   ...
   ElseIf modo = "M" Then
        ...
        vg_db.Execute "sgpadm_iu_recetacatdie_V02  'M', " & Val(Mid(TvwDir.Nodes(dest.Index).Key, 2, 20)) & ", '" & Mid(LimpiaDato(Trim(TvwDir.Nodes(dest.Index).text)), 1, 50) & "', 0, '" & vg_NUsr & "'"
...
ElseIf ivalidar = 0 Then
   ...
   If modo = "M" Then
      ...
      vg_db.CursorLocation = adUseClient
      Set RS = vg_db.Execute("sgpadm_Sel_CategoriaDietetica_V02 2," & codpadre & ",''")
      If Not RS.EOF Then codigo = RS!car_previo
      RS.Close
      Set RS = Nothing
   ...
   RS.CursorLocation = adUseClient
   vg_db.CursorLocation = adUseClient
   Set RS = vg_db.Execute("sgpadm_Sel_CategoriaDietetica_V02 6, " & codigo & ", '%" & UCase(LimpiaDato(Trim(TvwDir.Nodes(dest.Index).text))) & "%'")
   If Not RS.EOF Then
...
Set dest = TvwDir.SelectedItem
If TvwDir.SelectedItem.Index <> 1 Then

'   If Mid(TvwDir.Nodes(dest.Index).Parent.Key, 1, 1) = "H" Then
'
'      MsgBox "Solamente puede crear hasta dos niveles", vbExclamation + vbOKOnly, MsgTitulo
'      Exit Sub
'
'   End If
...
Set RS = vg_db.Execute("sgpadm_Sel_CategoriaDietetica_V02 2, " & Val(Mid(TvwDir.Nodes(dest.Index).Key, 2, 20)) & ",''")
...
If Not RS.EOF Then
   ...
   Set RS1 = vg_db.Execute("sgpadm_Sel_CategoriaDietetica_V02 5, " & RS!car_codigo & ",''")
...
If MsgBox(IIf(TvwDir.Nodes(dest.Index).BackColor = vbRed, "Activar", "Desactivar") & " registro...", vbQuestion + vbYesNo, MsgTitulo) = vbYes Then
   ...
   RS.Open "sgpadm_Sel_CategoriaDietetica_V02 2, " & Val(Mid(TvwDir.Nodes(dest.Index).Key, 2, 20)) & ",''", vg_db, adOpenStatic
   
   If Not RS.EOF Then
   
      CodPat = RS!car_previo
   ...
   Set RS = vg_db.Execute("sgpadm_Upd_DesactivarCategoriaDietetica_V01 " & Val(Mid(TvwDir.Nodes(dest.Index).Key, 2, 20)) & ", '" & Activo & "'")
...
If MsgBox("Cancela registro...", vbQuestion + vbYesNo, MsgTitulo) = vbYes Then
   ...
   Else
      ...
      Set RS = vg_db.Execute("sgpadm_Sel_CategoriaDietetica_V02 2, " & Val(Mid(TvwDir.Nodes(dest.Index).Key, 2, 20)) & ",''")
      If Not RS.EOF Then TvwDir.Nodes(dest.Index).text = Trim(RS!car_nombre)
      RS.Close
      Set RS = Nothing
      TvwDir.SetFocus
...
Set RS = vg_db.Execute("sgpadm_Sel_CategoriaDietetica_V02 3, " & Val(Mid(TvwDir.Nodes(dest1.Index).Key, 2, 20)) & ", ''")
If Not RS.EOF Then
   
   Do While Not RS.EOF
      
      Set nd = TvwDir.Nodes.Add(dest1.Index, tvwChild, "H" & RS!car_codigo, RS!car_nombre)
