...
Dim SQL As String, v_fecinv  As Variant, v_codbod As Long, sqlTP As String, sqlOT As String
fg_carga ""
v_codbod = fg_codigocbo(M_TomInv.Combo1, 0, 1, 0)
...
Case 1
    ...
    If Check1(1).Value = 0 And Check1(2).Value = 0 Then sqlOT = sqlOT & "and tin.tin_stofis<>0 and tin.tin_stosis<>0 "
    ...
    SQL = "select pro.pro_ctacon, pro.pro_codtip, tin.tin_codpro, pro.pro_nombre, tin.tin_propon, uni.uni_nomcor, tin.tin_stosis, tin.tin_stofis " & _
          "from b_tomainv tin, b_productos pro, a_unidad uni where tin.tin_codpro=pro.pro_codigo " & _
          "and pro.pro_coduni=uni.uni_codigo and tin.tin_fectom=" & v_fecinv & " and tin.tin_codbod=" & v_codbod & " " & sqlTP & sqlOT & "order by pro.pro_nombre"
    If optTIPLIS(0).Value = True Then I_Toma1 SQL
    If optTIPLIS(1).Value = True Then I_Toma2 SQL
    If optTIPLIS(2).Value = True Then I_Toma3 SQL
    If optTIPLIS(3).Value = True Then I_Toma4 SQL
    If optTIPLIS(4).Value = True Then I_Toma5 SQL
Case 3
    Me.Hide
    Unload Me
End Select
