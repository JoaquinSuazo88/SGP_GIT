...
Set RS = vg_db.Execute("sgpadm_s_cliente_V02 46, '" & LimpiaDato(fpText.text) & "', ''")
If RS.EOF Then
   RS.Close
   Set RS = Nothing
   fpayuda(0).Caption = ""
...
Dim Var_IndPpr As String
    Select Case Index
        Case 0
            ...
            vg_nombre = "": vg_codigo = ""
            Call B_TabEst.LlenaDatos("b_clientes", "cli_", "Clientes", "Cliente_EnvioBloque")
            B_TabEst.Show 1
            Me.Refresh
...
'            If Est Then MsgBox "Falta seleccionar concepto estructuras...", vbExclamation + vbOKOnly, Msgtitulo: Exit Sub ' Comenté esta linea, para evitar que termine el proceso si no han seleccionado todos los item Samuel 02/09/09
            ...
            Set RS = vg_db.Execute("sgpadm_s_LiveraMinutaSitRem_V02 2, '" & Trim(LimpiaDato(fpText.text)) & "', 0, 0, 0, 0, 0")
            If RS.EOF Then
                RS.Close
                Set RS = Nothing
                fpText.text = ""
            ...
            Set RS = vg_db.Execute("sgpadm_s_LiveraMinutaSitRem_V02 3, '', " & fpLongInteger1(0).text & ", 0, 0, 0, 0")
            If RS.EOF Then
                RS.Close: Set RS = Nothing
                fpLongInteger1(0).Value = ""
                fpayuda(1).Caption = ""
            ...
            Set RS = vg_db.Execute("sgpadm_s_LiveraMinutaSitRem_V02 4, '', " & fpLongInteger1(1).text & ", 0, 0, 0, 0")
            If RS.EOF Then
                RS.Close
            ...
            Set RS = vg_db.Execute("sgpadm_s_LiveraMinutaSitRem_V02 8, '', " & fpLongInteger1(2).text & ", " & fpLongInteger1(3).text & ", " & fecdes1 & ", " & Val(fg_codigocbo(Combo2, 0, 1, "")) & ", 0")
            ...
            Set RS = vg_db.Execute("sgpadm_s_LiveraMinutaSitRem_V02 9, '" & Trim(LimpiaDato(fpText.text)) & "', " & fpLongInteger1(0).text & ", " & fpLongInteger1(1).text & ", " & fecori1 & ", 0, 0")
            If RS.EOF Then
                RS.Close
                Set RS = Nothing
            ...
            '-------> Traer fecha minima de la minuta origen
            Set RS = vg_db.Execute("sgpadm_Sel_FechaMinimaMinBloque '" & Trim(LimpiaDato(fpText.text)) & "', " & Val(fpLongInteger1(0).Value) & ", " & Val(fpLongInteger1(1).Value) & ", " & Val(Format(fpDateTime1(0).text, "yyyymm")) & " ")
            If Not RS.EOF Then
               fecdes1 = IIf(IsNull(RS!min_fecmin), 0, RS!min_fecmin)
            End If
            RS.Close: Set RS = Nothing
            '-------> Validar si fecha destino
            If fecdes1 = 0 Then
                Call MsgBox("No existe datos origen, proceso cancelado", vbCritical + vbOKOnly, MsgTitulo)
                Exit Sub
            End If
            '-------> validar si Existe Datos Destino
            Set RS = vg_db.Execute("sgpadm_s_LiveraMinutaSitRem_V02 10, '" & fpLongInteger1(4).text & "', " & fpLongInteger1(2).text & ", " & fpLongInteger1(3).text & ", " & fecori1 & ", " & Val(fg_codigocbo(Combo2, 0, 1, "")) & ", " & fecdes2)
            Let Resp = "S"
            If Not RS.EOF Then
...
'                    Let Resp = "N"
            ...
            Set RS = vg_db.Execute("sgpadm_s_LiveraMinutaSitRem_V02 11, '" & Trim(LimpiaDato(fpText.text)) & "', " & fpLongInteger1(0).text & ", " & fpLongInteger1(1).text & ", " & fecori1 & ", 0, " & fecori2)
            If RS.EOF Then
                RS.Close
                Set RS = Nothing
           ...
           Sql = ""
           Sql = "sgpadm_p_copiacreaplanifSitRem '" & Trim(LimpiaDato(fpText.text)) & "', " & Val(fpLongInteger1(0).Value) & _
...
Private Function ValidaDestino() As Boolean
    ...
    Set RS = vg_db.Execute("sgpadm_s_LiveraMinutaSitRem_V02 5, '', " & fpLongInteger1(4).text & ", 0, 0,0, 0")
        If RS.EOF Then
            RS.Close
            Set RS = Nothing
    ...
    Set RS = vg_db.Execute("sgpadm_s_LiveraMinutaSitRem_V02 6, '', " & fpLongInteger1(2).text & ", 0, 0, 0, 0")
        If RS.EOF Then
            RS.Close
            Set RS = Nothing
            fpLongInteger1(2).Value = ""
    ...
    Set RS = vg_db.Execute("sgpadm_s_LiveraMinutaSitRem_V02 7, '', 0, " & fpLongInteger1(3).text & ", 0, 0, 0")
        If RS.EOF Then
            RS.Close
            Set ConSql = Nothing
            fpLongInteger1(3).Value = ""
    ...
    Set RS = vg_db.Execute("sgpadm_s_LiveraMinutaSitRem_V02 12, '" & Trim(LimpiaDato(fpText.text)) & "', " & fpLongInteger1(0).text & ", " & fpLongInteger1(1).text & ", " & Format(dBoM("01/" & fpDateTime1(0).text), "yyyymmdd") & ", 0, " & Format(dEoM("01/" & fpDateTime1(0).text), "yyyymmdd") & "")
        If RS.EOF Then
            RS.Close
            Set ConSql = Nothing
...
'Mover estructura servicio Origen
        ...
        Set RS = vg_db.Execute("sgpadm_s_LiveraMinutaSitRem_V02 1, '" & Trim(LimpiaDato(fpText.text)) & "', 0," & Val(fpLongInteger1(3).Value) & ", 0, 0, 0")
        If Not RS.EOF Then
            For i = 1 To vaSpread1.MaxRows
                vaSpread1.Row = i
                vaSpread1.Col = 5
