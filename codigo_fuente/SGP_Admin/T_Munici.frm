...
vaSpread1.Col = 3
retobl = IIf(vaSpread1.text = "1", "1", "0")
...
If modo = "A" Then
   ...
   codigo = 0
   Set RS = vg_db.Execute("sgpadm_iu_municipio 'A', 0, '" & Trim(Nombre) & "', '" & retobl & "'")
   If Not RS.EOF Then
...
If Combo1.ItemData(Combo1.ListIndex) = 0 Then
   ...
   RS.Open "sgpadm_s_municipio 3, 0, '%" & UCase(LimpiaDato(fpText1.text)) & "%'", vg_db, adOpenStatic

ElseIf Combo1.ItemData(Combo1.ListIndex) = 1 Then
    
   RS.Open "sgpadm_s_municipio 4, 0, '%" & UCase(LimpiaDato(fpText1.text)) & "%'", vg_db, adOpenStatic
...
If Not RS.EOF Then
   ...
   Do While Not RS.EOF
      ...
      vaSpread1.Value = RS!mun_codigo
      
      vaSpread1.Col = 2
      vaSpread1.Value = IIf(IsNull(RS!mun_nombre), "", Trim(RS!mun_nombre))
      
      vaSpread1.Col = 3
      vaSpread1.Value = IIf(IsNull(RS!mun_retobl) Or RS!mun_retobl = "0", "0", Trim(RS!mun_retobl))
      RS.MoveNext
...
RS.CursorLocation = adUseClient
vg_db.CursorLocation = adUseClient
Set RS = vg_db.Execute("sgpadm_s_municipio 2, 0, ''")
...
RS.CursorLocation = adUseClient
vg_db.CursorLocation = adUseClient
Set RS = vg_db.Execute("sgpadm_s_municipio 1, " & codigo & ", ''")
If Not RS.EOF Then
