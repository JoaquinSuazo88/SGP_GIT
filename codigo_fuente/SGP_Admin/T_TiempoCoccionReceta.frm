...
Begin VB.Form T_TiempoCoccionReceta 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Tiempo Coccion Receta"
   ClientHeight    =   4950
   ClientLeft      =   2940
   ClientTop       =   1110
   ClientWidth     =   8280
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.ComboBox Combo1 
         Appearance      =   0  'Flat
         Height          =   315
         ItemData        =   "T_TiempoCoccionReceta.frx":0000
         Left            =   2010
         List            =   "T_TiempoCoccionReceta.frx":000A
   ...
   Begin FPSpread.vaSpread vaSpread1 
      ...
      MaxCols         =   6
      ProcessTab      =   -1  'True
      SpreadDesigner  =   "T_TiempoCoccionReceta.frx":001E
      ScrollBarTrack  =   3
      ClipboardOptions=   0
   End
End
Attribute VB_Name = "T_TiempoCoccionReceta"
...
If modo = "A" Then
   ...
   codigo = 0
   Set RS = vg_db.Execute("sgpadm_Ins_TiempoCoccionReceta '" & Hora & "', '" & vg_NUsr & "'")
   If Not RS.EOF Then
      ...
      If RS(0) = 0 Then
         ...
         MensajeNuevo = codigo & ";" & Hora & ";" & 1 & ";" & vg_NUsr
...
Else
   ...
   Set RS = vg_db.Execute("sgpadm_Sel_CodigoTiempoCoccionReceta " & codigo & "")
   If Not RS.EOF Then
       
      MensajeAnterior = codigo & ";" & IIf(IsNull(RS!Hora), "", Trim(RS!Hora)) & ";" & IIf(IsNull(RS!Activo), "", Trim(RS!Activo)) & ";" & vg_NUsr
   ...
   Set RS = vg_db.Execute("sgpadm_Upd_TiempoCoccionReceta " & codigo & ", '" & Hora & "', '" & Activo & "', '" & vg_NUsr & "'")

   If Not RS.EOF Then
      
      If RS(0) = 0 Then
           
         MensajeNuevo = codigo & ";" & Hora & ";" & Activo & ";" & vg_NUsr
...
Me.Height = 5385
Me.Width = 11820
MsgTitulo = "Tiempo Coccion Receta"
...
If Combo1.ItemData(Combo1.ListIndex) = 0 Then
   ...
   Set RS = vg_db.Execute("sgpadm_Sel_BuscarCodigoTiempoCoccionReceta '%" & UCase(LimpiaDato(fpText1.text)) & "%'")

ElseIf Combo1.ItemData(Combo1.ListIndex) = 1 Then
    
   Set RS = vg_db.Execute("sgpadm_Sel_BuscarNombreTiempoCoccionReceta '%" & UCase(LimpiaDato(fpText1.text)) & "%'")
...
If Not RS.EOF Then
   ...
   Do While Not RS.EOF
      ...
      vaSpread1.Value = RS!IdTiempoCoccion
      
      vaSpread1.Col = 2
      vaSpread1.CellType = 6
      vaSpread1.TypeTime24Hour = 1
      vaSpread1.TypeTimeSeconds = False
      vaSpread1.TypeTimeSeparator = Asc(":")
      vaSpread1.TypeTimeMin = "000000"
      vaSpread1.TypeTimeMax = "235959"
      vaSpread1.Value = IIf(IsNull(RS!Hora), "", Replace(Trim(RS!Hora), ":", "")) & "00"
...
Case 5
    ...
    Mensaje = ""
    Mensaje = codigo & ";" & Hora & ";" & Activo & vg_NUsr
    ...
    Set RS = vg_db.Execute("sgpadm_Del_TiempoCoccionReceta " & codigo & ", '0', '" & vg_NUsr & "'")
...
Set RS = vg_db.Execute("sgpadm_Sel_TiempoCoccionReceta")
