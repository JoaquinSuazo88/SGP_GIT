...
With vaSpread1
    ...
    .Col = 3: Tipo = Trim(LimpiaDato(Left(.TypeCheckText, 1)))
    If Trim(Nombre) = "" Then MsgBox "Falta información...", vbExclamation + vbOKOnly, MsgTitulo: .Row = Fila: .Col = 2: .SetActiveCell 2, .MaxRows: .SetFocus: est = False: Exit Sub
    If modo = "A" Then
        vg_db.BeginTrans
        RS1.Open RutinaLectura.TipoAjuste(1, 0, "", 1), vg_db, adOpenStatic
        If Not RS1.EOF Then RS1.MoveFirst: codigo = RS1!aju_codigo + 1 Else codigo = 1
...
If Combo1.ItemData(Combo1.ListIndex) = 0 Then
    RS2.Open RutinaLectura.TipoAjuste(3, 0, UCase(LimpiaDato(fpText1.text)), 1), vg_db, adOpenStatic
ElseIf Combo1.ItemData(Combo1.ListIndex) = 1 Then
    RS2.Open RutinaLectura.TipoAjuste(4, 0, UCase(LimpiaDato(fpText1.text)), 1), vg_db, adOpenStatic
End If
With vaSpread1
    ...
    If Not RS2.EOF Then
       Do While Not RS2.EOF
          ...
          .Value = RS2!aju_codigo
          .Col = 2
          .Value = Trim(RS2!aju_nombre)
          .Col = 3
          .Value = IIf(RS2!aju_tipo = "D", 0, 1)
          est = False
          RS2.MoveNext
       Loop
       Gl_Ac_Botones Me, 1, IIf(vg_modprod = True, 1, 6), modo
...
With vaSpread1
    .MaxRows = 0
    RS2.Open RutinaLectura.TipoAjuste(2, 0, "", 1), vg_db, adOpenStatic
    Do While Not RS2.EOF
        .MaxRows = .MaxRows + 1
        .Row = .MaxRows
        est = True
        .Col = 1: .TypeHAlign = TypeHAlignRight: .Value = RS2!aju_codigo
        .Col = 2: .Value = Trim(RS2!aju_nombre)
        .Col = 3: .Value = IIf(RS2!aju_tipo = "D", 0, 1)
        est = False
        If RS2!aju_codigo = 3 Then .Col = -1: .Lock = True
        RS2.MoveNext
    Loop
    RS2.Close: Set RS2 = Nothing
    Gl_Ac_Botones Me, 1, IIf(vg_modprod = True, 1, 6), modo
...
If Not est And Row <> NewRow And NewRow > 0 And (modo = "A" Or modo = "M") And Toolbar1.Buttons(12).Visible = True Then
   GrabaRegistro Row
...
With vaSpread1
    .Row = .ActiveRow
    .Col = 1: codigo = Val(.Value)
    RS1.Open RutinaLectura.TipoAjuste(2, codigo, "", 1), vg_db, adOpenStatic
    If Not RS1.EOF Then
       .Col = 2: .Value = Trim(RS1!aju_nombre)
       .Col = 3: .Value = IIf(RS1!aju_tipo = "D", 0, 1)
    End If
    RS1.Close: Set RS1 = Nothing
End With
est = False
