...
Dim sqlTIP As String, sqlTIPB As String, sqlALE As String
...
Case 0 'Tipo Producto
    ...
    If Combo1(0).ListIndex > -1 Then sqlTIP = "AND pro.pro_codtip = " & codtip & ""
    MuestraGrilla
...
If Combo1(1).ItemData(Combo1(1).ListIndex) = 0 Then
   sqlALE = "AND pro.pro_codigo like '%" & UCase(LimpiaDato(fpText.text)) & "%'"
ElseIf Combo1(1).ItemData(Combo1(1).ListIndex) = 1 Then
   sql1 = IIf(vg_tipbase = "1", " Ucase(pro.pro_nombre) ", " UPPER(pro.pro_nombre) ")
   sqlALE = "AND " & sql1 & " like '%" & UCase(LimpiaDato(fpText.text)) & "%'"
End If
MuestraGrilla
End Sub
...
Dim sqlFIN As String, sqlFINB As String
sqlFIN = "SELECT DISTINCT pro.pro_codigo, pro.pro_nombre FROM b_productos pro, a_tiposervicio b, b_clientes c WHERE (b.tis_codigo = c.cli_codtis OR pro.pro_maepro < 1) AND c.cli_codigo = '" & MuestraCasino(1) & "' AND (b.tis_codigo = pro.pro_maepro OR pro.pro_maepro < 1) AND pro.pro_ctrsto = 1 AND (pro.pro_fecven > " & Format(Date, "yyyymmdd") & " OR pro.pro_fecven <= 0) "
sqlFINB = " UNION SELECT DISTINCT pro_codigo, pro_nombre FROM b_productos pro, b_bodegas bod, a_tiposervicio b, b_clientes c WHERE (b.tis_codigo = c.cli_codtis OR pro.pro_maepro < 1) AND c.cli_codigo = '" & MuestraCasino(1) & "' AND (b.tis_codigo = pro.pro_maepro OR pro.pro_maepro < 1) AND pro.pro_codigo = bod.bod_codpro AND bod.bod_codbod = " & vg_bodega & " AND bod.bod_canmer > 0 "
If sqlALE <> "" And sqlTIP = "" Then sqlFIN = sqlFIN & sqlALE & "": sqlFINB = sqlFINB & sqlALE & ""
If sqlALE = "" And sqlTIP <> "" Then sqlFIN = sqlFIN & sqlTIP & " ": sqlFINB = sqlFINB & sqlTIP & " "
'If sqlALE <> "" And sqlTIP <> "" Then sqlFIN = sqlFIN & sqlALE & " And " & sqlTIP": sqlFINB = sqlFINB & " & sqlALE & " And " & sqlTIP"
If sqlALE <> "" And sqlTIP <> "" Then sqlFIN = sqlFIN & sqlALE & sqlTIP: sqlFINB = sqlFINB & sqlALE & sqlTIP
If vg_tipbase = "1" Then
   sqlFIN = sqlFIN & " ORDER BY pro_nombre " & sqlFINB & " ORDER BY pro_nombre "
Else
   sqlFIN = sqlFIN & " " & sqlFINB & " ORDER BY pro_nombre "
End If
...
Do While Not RS1.EOF
    ...
    vaSpread1.Col = 2: vaSpread1.TypeHAlign = TypeHAlignRight: vaSpread1.CellType = 5
    vaSpread1.Value = Trim(RS1!pro_codigo)
    vaSpread1.Col = 3: vaSpread1.TypeHAlign = TypeHAlignLeft: vaSpread1.CellType = 5
