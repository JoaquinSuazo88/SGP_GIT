...
Begin VB.Form T_TipSer 
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.ComboBox Combo1 
         Appearance      =   0  'Flat
         Height          =   315
         ItemData        =   "T_TipSer.frx":0000
         Left            =   2010
         List            =   "T_TipSer.frx":000A
   ...
   Begin FPSpread.vaSpread vaSpread1 
      ...
      MaxCols         =   2
      ProcessTab      =   -1  'True
      SpreadDesigner  =   "T_TipSer.frx":001E
      ScrollBarTrack  =   3
      ClipboardOptions=   0
   End
...
With vaSpread1
    ...
    If Trim(Nombre) = "" Then MsgBox "Falta información...", vbExclamation + vbOKOnly, Msgtitulo: .Row = Fila: .Col = 2: .SetActiveCell 2, .Row: .SetFocus: OpGr = False: Exit Sub
    If modo = "A" Then
        vg_db.BeginTrans
        RS1.Open RutinaLectura.TipoServicio(1, 0, ""), vg_db, adOpenStatic
        If Not RS1.EOF Then RS1.MoveFirst: codigo = RS1!tis_codigo + 1 Else codigo = 1
        RS1.Close: Set RS1 = Nothing
        vg_db.Execute "INSERT INTO a_tiposervicio (tis_codigo, tis_nombre) VALUES (" & codigo & ", '" & Trim(Nombre) & "')"
        vg_db.CommitTrans
        .Col = 1: .Value = codigo
    Else
        vg_db.BeginTrans
        vg_db.Execute "UPDATE a_tiposervicio SET tis_nombre='" & Trim(Nombre) & "' WHERE tis_codigo=" & codigo
        vg_db.CommitTrans
    End If
...
If Combo1.ItemData(Combo1.ListIndex) = 0 Then
    RS2.Open RutinaLectura.TipoServicio(3, 0, UCase(LimpiaDato(fpText1.text))), vg_db, adOpenStatic
ElseIf Combo1.ItemData(Combo1.ListIndex) = 1 Then
    RS2.Open RutinaLectura.TipoServicio(4, 0, UCase(LimpiaDato(fpText1.text))), vg_db, adOpenStatic
End If
With vaSpread1
    ...
    If Not RS2.EOF Then
       Do While Not RS2.EOF
          .Row = i
          i = i + 1
          .Col = 1: .Value = RS2!tis_codigo
          .Col = 2: .Value = Trim(RS2!tis_nombre)
          RS2.MoveNext
       Loop
       Gl_Ac_Botones Me, 1, IIf(vg_modprod = True, 1, 6), modo
    End If
...
Case 5
    ...
    vg_db.BeginTrans
    vg_db.Execute "DELETE a_tiposervicio FROM a_tiposervicio WHERE tis_codigo=" & codigo
    vg_db.CommitTrans
...
With vaSpread1
    .MaxRows = 0
    RS2.Open RutinaLectura.TipoServicio(2, 0, ""), vg_db, adOpenStatic
    Do While Not RS2.EOF
        .MaxRows = .MaxRows + 1
        .Row = .MaxRows
        .Col = 1: .Value = RS2!tis_codigo
...
With vaSpread1
    .Row = .ActiveRow
    .Col = 1: codigo = Val(.Value)
    RS1.Open RutinaLectura.TipoServicio(2, codigo, ""), vg_db, adOpenStatic
    If Not RS1.EOF Then
       .Col = 2: .Value = Trim(RS1!tis_nombre)
    End If
    RS1.Close: Set RS1 = Nothing
End With
OpGr = False
