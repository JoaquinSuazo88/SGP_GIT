...
Begin VB.Form T_Region 
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.ComboBox Combo1 
         Appearance      =   0  'Flat
         Height          =   315
         ItemData        =   "T_Region.frx":0000
         Left            =   2010
         List            =   "T_Region.frx":000A
   ...
   Begin FPSpread.vaSpread vaSpread1 
      ...
      MaxCols         =   2
      ProcessTab      =   -1  'True
      SpreadDesigner  =   "T_Region.frx":001E
      ScrollBarTrack  =   3
      ClipboardOptions=   0
   End
...
If modo = "A" Then
   Codigo = 0
   Set RS = vg_db.Execute("sgpadm_iu_region 'A', 0, '" & Trim(Nombre) & "'")
   If Not RS.EOF Then
      Codigo = RS!indice
...
Else
    vg_db.Execute "sgpadm_iu_region 'M', " & Codigo & ", '" & Trim(Nombre) & "'"
End If
...
If Combo1.ItemData(Combo1.ListIndex) = 0 Then
    RS.Open "SELECT reg_codigo, reg_nombre FROM a_region WHERE reg_codigo LIKE '%" & UCase(LimpiaDato(fpText1.text)) & "%'", vg_db, adOpenStatic
ElseIf Combo1.ItemData(Combo1.ListIndex) = 1 Then
    RS.Open "SELECT reg_codigo, reg_nombre FROM a_region WHERE UPPER(reg_nombre) LIKE '%" & UCase(LimpiaDato(fpText1.text)) & "%'", vg_db, adOpenStatic
End If
...
If Not RS.EOF Then
   Do While Not RS.EOF
      vaSpread1.Row = i
      i = i + 1
      vaSpread1.Col = 1: vaSpread1.Lock = True: vaSpread1.TypeHAlign = TypeHAlignLeft: vaSpread1.text = RS!reg_codigo
      vaSpread1.Col = 2: vaSpread1.Lock = False: vaSpread1.TypeHAlign = TypeHAlignLeft: vaSpread1.text = IIf(IsNull(RS!reg_nombre), "", Trim(RS!reg_nombre))
      RS.MoveNext
   Loop
   Gl_Ac_Botones Me, 1, 1, modo
End If
...
Case 5
    ...
    vaSpread1.Col = 1: Codigo = Val(vaSpread1.text)
    vg_db.Execute "DELETE a_region FROM a_region WHERE reg_codigo = " & Codigo & ""
...
vaSpread1.Visible = False
vaSpread1.MaxRows = 0
RS.Open "SELECT reg_codigo, reg_nombre FROM a_region ORDER BY reg_codigo", vg_db, adOpenStatic
Do While Not RS.EOF
    vaSpread1.MaxRows = vaSpread1.MaxRows + 1
    vaSpread1.Row = vaSpread1.MaxRows
    vaSpread1.Col = 1: vaSpread1.Lock = True: vaSpread1.text = IIf(IsNull(RS!reg_codigo), "", RS!reg_codigo)
    vaSpread1.Col = 2: vaSpread1.Lock = False: vaSpread1.text = IIf(IsNull(RS!reg_nombre), "", Trim(RS!reg_nombre))
    RS.MoveNext
Loop
RS.Close: Set RS = Nothing
Gl_Ac_Botones Me, 1, 1, modo
...
vaSpread1.Row = vaSpread1.ActiveRow
vaSpread1.Col = 1: Codigo = Val(vaSpread1.text)
RS.Open "SELECT * FROM a_region WHERE reg_codigo = " & Codigo & "", vg_db, adOpenStatic
If Not RS.EOF Then
   vaSpread1.Col = 2: vaSpread1.Value = IIf(IsNull(RS!reg_nombre), "", Trim(RS!reg_nombre))
End If
RS.Close: Set RS = Nothing
OpGr = False
End Sub
