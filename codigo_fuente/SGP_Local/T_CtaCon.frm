...
OpGr = True
vaSpread1.Row = Fila
vaSpread1.Col = 1: codigo = LimpiaDato(vaSpread1.Value)
...
If modo = "A" Then
    vg_db.BeginTrans
    vg_db.Execute "INSERT INTO a_ctacontable (cta_codigo, cta_nombre) VALUES (" & codigo & ", '" & Trim(Nombre) & "')"
    vg_db.CommitTrans
    vaSpread1.Col = 1: vaSpread1.Value = codigo
    MoverDatosGrillas
Else
    vg_db.BeginTrans
    vg_db.Execute "UPDATE a_ctacontable SET cta_nombre='" & Trim(Nombre) & "' WHERE cta_codigo='" & Trim(codigo) & "'"
    vg_db.CommitTrans
End If
...
If Combo1.ItemData(Combo1.ListIndex) = 0 Then
    RS.Open RutinaLectura.CuentaContable(2, "", UCase(LimpiaDato(fpText1.text))), vg_db, adOpenStatic
ElseIf Combo1.ItemData(Combo1.ListIndex) = 1 Then
    RS.Open RutinaLectura.CuentaContable(3, "", UCase(LimpiaDato(fpText1.text))), vg_db, adOpenStatic
End If
With vaSpread1
    ...
    If Not RS.EOF Then
       Do While Not RS.EOF
          .Row = i
          i = i + 1
          .Col = 1: .Value = RS!cta_codigo
          .Col = 2: .Value = IIf(IsNull(RS!cta_nombre), "", Trim(RS!cta_nombre))
          RS.MoveNext
       Loop
...
With vaSpread1
    .MaxRows = 0
    RS.Open RutinaLectura.CuentaContable(1, "", ""), vg_db, adOpenStatic
    Do While Not RS.EOF
       .MaxRows = .MaxRows + 1
       .Row = .MaxRows
       .Col = 1: .Value = RS!cta_codigo
       .Col = 2: .Value = Trim(RS!cta_nombre)
       RS.MoveNext
    Loop
    RS.Close: Set RS = Nothing
    Gl_Ac_Botones Me, 1, IIf(vg_modprod = True, IIf(.MaxRows > 0, 1, 2), 6), modo
...
RS.Open RutinaLectura.CuentaContable(1, codigo, ""), vg_db, adOpenStatic
If Not RS.EOF Then vaSpread1.Col = 2: vaSpread1.Value = IIf(IsNull(RS!cta_nombre), "", Trim(RS!cta_nombre))
RS.Close: Set RS = Nothing
OpGr = False
End Sub
