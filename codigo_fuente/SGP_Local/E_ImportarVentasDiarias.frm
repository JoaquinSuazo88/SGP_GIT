...
Begin VB.Form E_ImportarVentasDiarias 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Importación Ventas Diarias(Captura de las Q)"
   ClientHeight    =   2100
   ClientLeft      =   45
   ClientTop       =   390
   ClientWidth     =   7725
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.Image Image1 
         ...
         Index           =   0
         Left            =   2685
         Picture         =   "E_ImportarVentasDiarias.frx":0000
         Top             =   300
         Width           =   480
      End
   ...
   End
End
Attribute VB_Name = "E_ImportarVentasDiarias"
...
MsgTitulo = "Importar Ventas Diarias (Captura de las Q)"
...
fpText.text = Trim(RS!cli_codigo)
RS.Close
Set RS = Nothing
...
If Format(FpFecDesde.text, "yyyymmdd") > Format(FpFecHasta.text, "yyyymmdd") Then

   MsgBox "Fecha origen es menor que destino", vbExclamation + vbOKOnly, MsgTitulo
   ValidarDatos = False
   Exit Function

End If

If Mid(FpFecDesde.text, 4, 2) <> Mid(FpFecHasta.text, 4, 2) Then

   MsgBox "Fechas deben estar dentro de un mes", vbExclamation + vbOKOnly, MsgTitulo
   ValidarDatos = False
   Exit Function
   
End If

If Mid(FpFecDesde.text, 7, 4) <> Mid(FpFecHasta.text, 7, 4) Then
...
Select Case Button.Index
    ...
    Case 1
        ...
        I_ImportrarVentasDiarias
        
        Toolbar1.Enabled = True
    
    Case 3

        ExplorarCarpeta dir_trabajo_Inf & "SPRS_Plantilla_Carga_Masiva"
...
Set RS = vg_db.Execute("sgp_Sel_ImportarVentasDiarias '" & LimpiaDato(fpText.text) & "', '" & Format(FpFecDesde.text, "yyyymmdd") & "', '" & Format(FpFecHasta.text, "yyyymmdd") & "'")
...
NombreArchivoExcel = "SPRS-plantilla_carga_masiva_" & Trim(fpText.text) & "_" & Format(Date, "yyyymmdd") & "_" & Format(Time, "HHMMSS")
Generar_ArchivoExcelVta "sgp_Sel_ImportarVentasDiarias '" & LimpiaDato(fpText.text) & "', '" & Format(FpFecDesde.text, "yyyymmdd") & "', '" & Format(FpFecHasta.text, "yyyymmdd") & "'", dir_trabajo_Inf & "SPRS_Plantilla_Carga_Masiva\", NombreArchivoExcel
