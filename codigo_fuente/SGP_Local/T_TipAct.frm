...
Begin VB.Form T_TipAct 
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.ComboBox Combo1 
         Appearance      =   0  'Flat
         Height          =   315
         ItemData        =   "T_TipAct.frx":0000
         Left            =   2010
         List            =   "T_TipAct.frx":000A
   ...
   Begin FPSpread.vaSpread vaSpread1 
      ...
      MaxCols         =   2
      ProcessTab      =   -1  'True
      SpreadDesigner  =   "T_TipAct.frx":001E
      ScrollBarTrack  =   3
      ClipboardOptions=   0
   End
...
If modo = "A" Then
   Codigo = 0
   Set RS1 = vg_db.Execute("sgpadm_iu_tipoactividad 'A', 0, '" & Trim(Nombre) & "'")
   If Not RS1.EOF Then
      Codigo = RS1!indice
...
Else
    vg_db.Execute "sgpadm_iu_tipoactividad 'M', " & Codigo & ", '" & Trim(Nombre) & "'"
End If
...
If Err = 3034 Then Exit Sub 'vg_db.RollbackTrans: Exit Sub
'vg_db.RollbackTrans
fg_descarga
...
If Combo1.ItemData(Combo1.ListIndex) = 0 Then
    RS2.Open "SELECT tia_codigo, tia_nombre FROM a_tipoactividad WHERE tia_codigo LIKE '%" & UCase(LimpiaDato(fpText1.text)) & "%'", vg_db, adOpenStatic
ElseIf Combo1.ItemData(Combo1.ListIndex) = 1 Then
    RS2.Open "SELECT tia_codigo, tia_nombre FROM a_tipoactividad WHERE UPPER(tia_nombre) LIKE '%" & UCase(LimpiaDato(fpText1.text)) & "%'", vg_db, adOpenStatic
End If
...
If Not RS2.EOF Then
   Do While Not RS2.EOF
      vaSpread1.Row = i
      i = i + 1
      vaSpread1.Col = 1: vaSpread1.Lock = True: vaSpread1.TypeHAlign = TypeHAlignLeft: vaSpread1.text = RS2!tia_codigo
      vaSpread1.Col = 2: vaSpread1.Lock = False: vaSpread1.TypeHAlign = TypeHAlignLeft: vaSpread1.text = IIf(IsNull(RS2!tia_nombre), "", Trim(RS2!tia_nombre))
      RS2.MoveNext
   Loop
   Gl_Ac_Botones Me, 1, 1, modo
End If
...
Case 5
    ...
    vaSpread1.Col = 1: Codigo = Val(vaSpread1.text)
    vg_db.Execute "DELETE a_tipoactividad FROM a_tipoactividad WHERE tia_codigo = " & Codigo & ""
...
vaSpread1.Visible = False
vaSpread1.MaxRows = 0
RS2.Open "SELECT tia_codigo, tia_nombre FROM a_tipoactividad ORDER BY tia_codigo", vg_db, adOpenStatic
Do While Not RS2.EOF
    vaSpread1.MaxRows = vaSpread1.MaxRows + 1
    vaSpread1.Row = vaSpread1.MaxRows
    vaSpread1.Col = 1: vaSpread1.Lock = True: vaSpread1.text = IIf(IsNull(RS2!tia_codigo), "", RS2!tia_codigo)
    vaSpread1.Col = 2: vaSpread1.Lock = False: vaSpread1.text = IIf(IsNull(RS2!tia_nombre), "", Trim(RS2!tia_nombre))
    RS2.MoveNext
Loop
RS2.Close: Set RS2 = Nothing
Gl_Ac_Botones Me, 1, 1, modo
...
vaSpread1.Row = vaSpread1.ActiveRow
vaSpread1.Col = 1: Codigo = Val(vaSpread1.text)
RS1.Open "SELECT * FROM a_tipoactividad WHERE tia_codigo=" & Codigo & "", vg_db, adOpenStatic
If Not RS1.EOF Then
   vaSpread1.Col = 2: vaSpread1.Value = IIf(IsNull(RS1!tia_nombre), "", Trim(RS1!tia_nombre))
End If
RS1.Close: Set RS1 = Nothing
OpGr = False
End Sub
