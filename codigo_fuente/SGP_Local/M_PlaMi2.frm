...
Begin VB.Form M_Plami2 
   ...
   Begin FPSpread.vaSpread vaSpread1 
      ...
      Protect         =   0   'False
      RestrictRows    =   -1  'True
      SpreadDesigner  =   "M_PlaMi2.frx":0D60
      UserResize      =   1
      VisibleCols     =   1
      VisibleRows     =   100
      TextTip         =   2
...
Set BtnX = Toolbar1.Buttons.Add(, , "", tbrSeparator, 0): BtnX.Enabled = False
Set BtnX = Toolbar1.Buttons.Add(, "A_ExportaPlanif", , tbrDefault, "A_ExportaPlanif"): BtnX.Visible = False: Me.HelpContextID = 1032000: BtnX.Enabled = IIf(Mid(ValidarUsuarioAcceso(M_Plami2), 1, 1) = "0", False, True): BtnX.ToolTipText = IIf(Mid(ValidarUsuarioAcceso(M_Plami2), 1, 1) = "0", "", "Exportar Planificación Teórica")
Set BtnX = Toolbar1.Buttons.Add(, "A_ImportaPlanif", , tbrDefault, "A_ImportaPlanif"): BtnX.Visible = False: Me.HelpContextID = 1034000: BtnX.Enabled = IIf(Mid(ValidarUsuarioAcceso(M_Plami2), 1, 1) = "0", False, True): BtnX.ToolTipText = IIf(Mid(ValidarUsuarioAcceso(M_Plami2), 1, 1) = "0", "", "Importar Planificación")
Set BtnX = Toolbar1.Buttons.Add(, , "", tbrSeparator, 0): BtnX.Enabled = False
...
RS.Open RutinaLectura.Servicio(2, vg_codservicio, ""), vg_db, adOpenStatic
If Not RS.EOF Then nomser = Trim(RS!ser_nombre)
RS.Close: Set RS = Nothing
Label4.Caption = Trim(M_Plami1.fpayuda(0).Caption) & "(" & M_Plami1.fpText.text & ")" & " - " & nomreg & " - " & nomser
...
'If (vg_codregimen > 9999 And "S" = (fg_CambiaChar(GetParametro("5etapas"), ";", "','"))) Then
...
If MsgBox(" Actualiza planificación teórica...", vbQuestion + vbYesNo, MsgTitulo) = vbNo Then IndGrabado = 0: Cancel = -1: Me.Hide: Unload Me: M_Plami1.WindowState = 0
...
fg_carga ""
G_TeoRea.LlenarGrafico vg_codcasino, vg_codregimen & ",", vg_codservicio & ",", Val(vg_fecha) & "01", Format(dEoM("01/" & Mid(vg_fecha, 5, 2) & "/" & Mid(vg_fecha, 1, 4)), "yyyymmdd"), 0, 0, "'1'", 0, IIf(Index = 0, False, True)
G_TeoRea.Show 1
fg_descarga
...
Dim StrRec As String, StrRecb As String, aAp As String, sql1 As String
Dim i As Long, j As Long, X As Long, CodRec As Long, tiprec As Long
...
Case 5 '-------> Ver detalle recetas
    ...
    If vg_newestrec = True Then
       ...
       If vg_fecval < 10 Then vg_fecval = Val(vg_fecha) & "0" & vg_fecval Else vg_fecval = Val(vg_fecha) & vg_fecval
       RS.Open RutinaLectura.Minutas(7, vg_codregimen, vg_codservicio, vg_fecval, "1"), vg_db, adOpenStatic
       If Not RS.EOF Then vg_fecval = RS!mid_fecval: vg_opcion = 1
       RS.Close: Set RS = Nothing
    End If
    vg_newnomrec = ""
...
'mod 20050914            B_Receta.vaSpread1.Col = 4: B_Receta.vaSpread1.Text = IIf(vg_tiprec = 0, "Patrón", "Local")
'mod 20050914            B_Receta.vaSpread1.Col = 5: B_Receta.vaSpread1.Text = IIf(vg_tiprec = 0, 0, 1)
        End If
        IndGrabado = 1
...
Case 10 '-------> Calcular Aporte Día
    ...
    C_ApoPla.LlenarApoPlan M_Plami2, "Aporte Planificación Teórica " & vaSpread1.text, vg_codcasino, vg_codregimen, vg_codservicio, Val(vg_fecha), 1, j
    C_ApoPla.Show 1, Me
...
Case 12 '-------> Frecuencia recetas en planificación
    ...
    'C_FrePla.LlenarFrecPlan "Frecuencia Planificación Teórica " & Mid(vg_fecha, 5, 2) & "/" & Mid(vg_fecha, 1, 4), vg_codcasino, vg_codregimen, vg_codservicio, Val(vg_fecha), 1
    C_FrePla.Show 1, Me
Case 13 '-------> Actualizar Costo recetas
    ...
    If vg_tipbase = "1" Then
       ...
       RS.Open "SELECT DISTINCT a.pro_codigo, a.pro_nombre, c.uni_nombre " & _
               ...
               "AND   val(mid(d.min_fecmin,1,6))=" & vg_fecha & " " & _
    ...
    Else
       ...
       RS.Open "SELECT distinct a.pro_codigo, a.pro_nombre, c.uni_nombre " & _
               ...
               "AND   d.min_codreg = " & vg_codregimen & " " & _
               "AND   d.min_codser = " & vg_codservicio & " " & _
               "AND   convert(int,substring(convert(varchar(8),d.min_fecmin),1,6)) = " & vg_fecha & " " & _
               "AND   e.mid_tipmin = '" & 1 & "'  " & _
               "AND   a.pro_coduni = c.uni_codigo " & _
    ...
    Else
       ...
       '-------> Borrar tablas temporales
       If Trim(aAp) <> "" And vg_tipbase = "1" Then vg_db.Execute "DROP TABLE " & aAp & ""
       M_ProPre.LlenarListaPrecio vg_codcasino, vg_codregimen, vg_codservicio, Val(vg_fecha), 1, 0
       M_ProPre.Show 1, Me
    End If
    fg_carga ""
    Dim vecactrec As Variant
    '-------> Traer total de receta desde planificación de minutas y luego calcular costo
    RS.Open RutinaLectura.Minutas(8, vg_codregimen, vg_codservicio, Val(vg_fecha), "1"), vg_db, adOpenStatic
    ...
    RS.Open RutinaLectura.Minutas(9, vg_codregimen, vg_codservicio, Val(vg_fecha), "1"), vg_db, adOpenStatic
...
'                    cosdes = fg_CalCtoRecInv(codrec, tiprec, (fg_CambiaChar(GetParametro("ctalimdes"), ";", "','")))
                    ...
                    If vg_tipbase = "1" Then
                       vg_db.Execute "UPDATE b_minutadet INNER JOIN b_minuta ON b_minutadet.mid_codigo = b_minuta.min_codigo SET b_minutadet.mid_cosrec = " & cosali & ", b_minutadet.mid_cosdes = " & CosDes & " " & _
                                     "WHERE b_minuta.min_cencos = '" & vg_codcasino & "' AND b_minuta.min_codreg = " & vg_codregimen & " AND b_minuta.min_codser = " & vg_codservicio & " AND b_minuta.min_fecmin = " & Val(Fecha) & " AND b_minutadet.mid_codrec = " & CodRec & " AND b_minutadet.mid_tiprec = " & tiprec & " AND b_minutadet.mid_numlin = " & j & " AND b_minutadet.mid_tipmin = '1'"
                    Else
                       vg_db.Execute "UPDATE b_minutadet SET b_minutadet.mid_cosrec = " & cosali & ", b_minutadet.mid_cosdes = " & CosDes & " FROM b_minuta,  b_minutadet WHERE b_minutadet.mid_codigo = b_minuta.min_codigo " & _
                                     "AND b_minuta.min_cencos = '" & vg_codcasino & "' AND b_minuta.min_codreg = " & vg_codregimen & " AND b_minuta.min_codser = " & vg_codservicio & " AND b_minuta.min_fecmin = " & Val(Fecha) & " AND b_minutadet.mid_codrec = " & CodRec & " AND b_minutadet.mid_tiprec = " & tiprec & " AND b_minutadet.mid_numlin = " & j & " AND b_minutadet.mid_tipmin = '1'"
                    End If
                    IndGrabado = 1
                  End If
              Next j
...
Case 14 '-------> Exportar recetas
    If IndGrabado = 1 Then MsgBox "Actualice Datos, para ver Información", vbInformation + vbOKOnly, MsgTitulo: Exit Sub
    C_ExpRec.LlenarExporReceta "Exportar Recetas " & Mid(vg_fecha, 5, 2) & "/" & Mid(vg_fecha, 1, 4), vg_codcasino, vg_codregimen, vg_codservicio, Val(vg_fecha), 1
    C_ExpRec.Show 1, Me
...
Dim Del_Row As Integer, indcol As Integer, indrow As Integer, IndCol2 As Integer, IndRow2 As Integer, indrow3 As Integer
Dim FilaAct As Long, FilaAnt As Long, FilaPos As Long, AuxIblockrow As Integer, AddRec As Long, codest As Long
Dim cosali As Double, CosDes As Double
Dim AadRec As Long
Dim fil  As Long
Dim Colu As Long
...
Case 2 '-------> Ingresar recetas
    ...
    vaSpread1.Col = j - 1
    vaSpread1.Row = vaSpread1.ActiveRow
    If vaSpread1.BackColor = &H80FF80 And etapa5 Then MsgBox "No puede modificar receta, corresponde 5 etapas", vbCritical + vbOKOnly, MsgTitulo: Exit Sub
    AadRec = 0
...
'       RS.Open "select sra_serdia, sum(sra_raciones) as sra_raciones from a_serviciorac where sra_codser=" & vg_codservicio & " and sra_serdia=" & IIf(fg_Dia(Format(CDate(Mid(Trim(vaSpread1.Text), 5, Len(Trim(vaSpread1.Text)))), "yyyymmdd")) = 1, 7, (fg_Dia(Format(CDate(Mid(Trim(vaSpread1.Text), 5, Len(Trim(vaSpread1.Text)))), "yyyymmdd")) - 1)) & " group by sra_serdia", vg_db, adOpenStatic
       ...
       Next i
       Set RS = vg_db.Execute(RutinaLectura.EstServicio(2, vg_codservicio, codest))
       vaSpread1.Row = vaSpread1.ActiveRow
       vaSpread1.Col = j + 1
...
Case 5 '-------> Insertar linea
    ...
    If Not ValidarAccesoMinutaBloqueyBloqueo(vg_codcasino, 1) Then
       MsgBox "Minuta, sin acceso modificaciones", vbCritical + vbOKOnly, MsgTitulo: Exit Sub
    Else
        RS.Open "SELECT DISTINCT ped_codcas FROM b_minutapedidos WHERE ped_codcas = '" & vg_codcasino & "' AND ped_anomes = " & Val(vg_fecha), vg_db, adOpenStatic
        If Not RS.EOF Then
            RS.Close
            Set RS = Nothing
        ...
        vaSpread1.BackColor = IIf(RS.EOF, Shape1(0).FillColor, Shape1(1).FillColor)
        RS.Close: Set RS = Nothing
    End If
'    vaSpread1.MaxRows = vaSpread1.MaxRows + ((IblockRow2 - IblockRow) + 1) '1
'    vaSpread1.InsertRows IblockRow, ((IblockRow2 - IblockRow) + 1)
'    vaSpread1.Row = IblockRow: vaSpread1.Col = -1:
'    vaSpread1.Row = IblockRow: vaSpread1.Col = 1:
    vaSpread1.Row = xRowIni: vaSpread1.Col = 1
       ...
       Next X
        For j = IblockCol To iblockcol2
           ...
           For i = IblockRow To (IblockRow2 + (AiBlockRow2 - AiBlockRow))
              vaSpread1.Row = i
              If vaSpread1.BackColor = &H80FF80 Then MsgBox "No puede modificar receta, corresponde 5 etapas", vbCritical + vbOKOnly, MsgTitulo: Exit Sub
           Next i
           For i = 1 To vaSpread1.MaxRows - 1
              vaSpread1.Row = i
              If vaSpread1.BackColor = &HFFFF00 Then
...
'    If vaSpread1.Text <> "N.Rac." Then
    ...
    Else
       vaSpread1.Row = 0
       For i = 1 To MaxColumna
           If (VectorCol(i) - 1) = IblockCol Or VectorCol(i) = IblockCol Or (VectorCol(i) + 1) = IblockCol Or (VectorCol(i) + 2) = IblockCol Then
...
'       ReDim VecRacPegar(0)
    ...
    If Index = 14 And IndCortarPegar = 1 Then
       ...
       If etapa5 And AddReceta > 0 Then
          For j = 1 To UBound(VectorCol)
              ...
              For i = 1 To vaSpread1.MaxRows - 1
                  ...
                  If vaSpread1.BackColor = &HFFFF00 Then
                     AddRec = AddRec + 1
                     If AddRec >= AddReceta Then vaSpread1.Col = VectorCol(j): vaSpread1.Row = SpreadHeader + IIf(vCtoPis > 0 And vCtoTec > 0, 3, IIf(vCtoPis < 1 And vCtoTec > 0, 2, IIf(vCtoPis > 0 And vCtoTec = 0, 2, 1))): nrodia = nrodia & Val(Mid(Format(CDate(Mid(Trim(vaSpread1.text), 5, Len(Trim(vaSpread1.text)))), "yyyymmdd"), 7, 2)) & ";": Exit For
                  End If
              Next i
          Next j
       End If
       ...
       For i = 3 To (vaSpread1.MaxCols - 1) Step 5
...
'                              vaSpread1.TypeIntegerMax = 9999999
                      ...
                      Next XX
                      vaSpread1.Col = X
                      If X = (i - 1) Then vaSpread1.ForeColor = &HFF&: vaSpread1.BackColor = IIf(Not etapa5, &H80FF80, &HFFFF00)
                  Next X
                  If L > 0 Then
                     z = L
...
'           For j = 0 To UBound(vecdia)
'               If nrodia = vecdia(j) Then
'                  vaSpread1.Row = aiblockrow: vaSpread1.Col = i
'                  If Trim(vaSpread1.Text) <> "" Then
'                     For X = aiblockrow + 1 To vaSpread1.MaxRows
'                         vaSpread1.Row = X: vaSpread1.Col = vaSpread1.MaxCols: xser = Val(vaSpread1.Text)
'                         vaSpread1.Col = i + 1
'                         If vaSpread1.Row = vaSpread1.MaxRows Then vaSpread1.MaxRows = vaSpread1.MaxRows + 1: vaSpread1.InsertRows X, 1: Exit For
'                         If xser <> iser And xser > 0 Then
'                            vaSpread1.MaxRows = vaSpread1.MaxRows + 1: vaSpread1.InsertRows X, 1: Exit For
'                         ElseIf Trim(vaSpread1.Text) <> "" And xser > 0 Then
'                            vaSpread1.MaxRows = vaSpread1.MaxRows + 1: vaSpread1.InsertRows X + 1, 1: X = X + 1: Exit For
'                         ElseIf Trim(vaSpread1.Text) = "" Then
'                            Exit For
'                         End If
'                     Next X
'                     vaSpread1.CopyRange aiblockcol, aiblockrow, aiblockcol2, aiblockrow2, i, X
'                  Else
'                     vaSpread1.CopyRange aiblockcol, aiblockrow, aiblockcol2, aiblockrow2, i, aiblockrow
'                  End If
'                  Exit For
'               End If
'           Next j
       ...
       For i = IblockCol To iblockcol2 Step 5
           If IndCortarPegar = 1 Then
              ...
              For j = vaSpread1.MaxRows - (AiBlockRow2 - AiBlockRow) To vaSpread1.MaxRows
                  ...
                  For X = (i) To (i) + 4
                      vaSpread1.Col = X
                      If TipoCopia <> "Copiar Raciones" And AiBlockRow <> MaxFila Then
                         vaSpread1.BackColor = Shape1(0).FillColor
                      End If
                      For XX = 1 To MaxColumna
                          If (VectorCol(XX) - 1) = vaSpread1.Col Then
                      ...
                      If X = (i) And Trim(vaSpread1.text) <> "" And TipoCopia <> "Copiar Raciones" Then
                         vaSpread1.ForeColor = &HFF&: vaSpread1.BackColor = IIf(Not etapa5, &H80FF80, &HFFFF00)
                      End If
                  Next X
              Next j
              'Fin asignar colores
...
'                If Trim(VecRacPegar(n1, 1)) = "" Then
'                  vaSpread1.Col = f - 1: desc = vaSpread1.text
'                  vaSpread1.Col = f: vaSpread1.Row = FilaActiva
'                  vaSpread1.text = IIf(Trim(desc) = "", "", 0)
'                ElseIf Trim(VecRacPegar(n1, 1)) = "0" Then
'                  vaSpread1.text = IIf(Trim(VecRacPegar(n1, 1)) = "0", VecSelGrid(n), VecRacPegar(n1, 1))
'                  vaSpread1.Col = ColumnaActiva + 1
'                  vaSpread1.text = VecRacPegar(n1, 2)
'                Else
'                  If TipoCopia = "Copiar Raciones" Then
...
'                        vaSpread1.text = Trim(VecSelGrid(n))
'                      Else
'                        vaSpread1.text = Trim(VecRacPegar(n1, 1))
'                      End If
'                    End If
'                    If n <= max Then n = n + 1
'                    If n1 <= max1 Then n1 = n1 + 1
...
'          vaSpread1.Col = f: vaSpread1.Row = SpreadHeader + IIf(vCtoPis > 0 And vCtoTec > 0, 3, IIf(vCtoPis < 1 And vCtoTec > 0, 2, IIf(vCtoPis > 0 And vCtoTec = 0, 2, 1))) '0
'          If vaSpread1.text = "N.Rac." Then
'              n = 1: n1 = 1: max = contador: max1 = contador_b
...
'                  vaSpread1.text = IIf(Trim(desc) = "", "", 0)
'                Else
'                  vaSpread1.text = IIf(Trim(VecRacPegar(n1, 1)) = 0, VecSelGrid(n), VecRacPegar(n1))
'                End If
'                If n <= max Then n = n + 1
'                If n1 <= max1 Then n1 = n1 + 1
'                If n > max Then n = 1
...
If vaSpread1.ChangeMade = True Then
   If Val(vaSpread1.text) = 0 And etapa5 Then
      Dim codest As Long
      codest = 0
      vaSpread1.Row = Row
      For i = (IIf(vaSpread1.Row = 1, 1, vaSpread1.Row + 1 - 1)) To 1 Step -1
...
Case 46
    ...
    If etapa5 And AddReceta > 0 Then
        For j = IblockCol To iblockcol2 Step 5
           ...
           For i = IblockRow To IblockRow2
              vaSpread1.Row = i
              If vaSpread1.BackColor = &H80FF80 Then MsgBox "No puede eliminar receta, corresponde 5 etapas", vbCritical + vbOKOnly, MsgTitulo: Exit Sub
          Next i
        Next j
    End If
...
Dim totrac          As Long
Dim rec5eta         As String, sql1 As String, sql2 As String, aAp As String
Dim cosali          As Double, CosDes As Double
...
'If vg_tipbase = "1" Then vg_db.BeginTrans
   ...
   For i = 2 To (vaSpread1.MaxCols - 1) Step 5
      ...
      If vaSpread1.BackColor <> Shape1(1).FillColor Then
         ...
         estser = 0
            ...
            For j = 1 To (vaSpread1.MaxRows - 1)
                ...
                desc = "": CodRec = 0
                cosali = 0: CosDes = 0: rec5eta = ""
                vaSpread1.Row = j
                vaSpread1.Col = vaSpread1.MaxCols
                If Trim(vaSpread1.text) <> "" Then estser = vaSpread1.text
                vaSpread1.Col = i: rec5eta = IIf(Not etapa5, "0", IIf(vaSpread1.BackColor = &HFFFF00, "0", "1"))
      ...
      Set RS = vg_db.Execute("sgp_Ins_XmlMinutaTeorica '" & MyBuffer & "', '" & vg_codcasino & "', " & vg_codregimen & ", " & vg_codservicio & ", " & FecMinuta & ", " & totrac & "")
...
vg_db.Execute "UPDATE b_minutadet SET mid_tiprec = -1 " & _
              "WHERE  mid_codigo IN (SELECT DISTINCT a.min_codigo FROM b_minuta a WHERE a.min_cencos = '" & vg_codcasino & "' AND " & sql1 & " = " & Val(vg_fecha) & ") " & _
              "AND    mid_codrec IN (SELECT DISTINCT b.mid_codrec FROM b_minuta a, b_minutadet b WHERE a.min_codigo = b.mid_codigo AND a.min_cencos = '" & vg_codcasino & "' AND " & sql1 & " = " & Val(vg_fecha) & " AND b.mid_tipmin = '1' AND b.mid_tiprec = -1) " & _
              "AND    mid_tiprec = 0 " & _
              "AND    mid_tipmin = '1'"
...
fg_carga ""
Dim indrow3 As Long, inddia As Long, Fecha As String, sql1 As String, indblo As Boolean, estfij As Boolean
...
With vaSpread1
    ...
    .Row = -1: .Col = -1:
    RS.Open RutinaLectura.Minutas(3, 0, 0, Val(vg_fecha), ""), vg_db, adOpenStatic
    If RS.EOF Then
       .BackColor = Shape1(0).FillColor
       indblo = True
    ElseIf Not ValidarAccesoMinutaBloqueyBloqueo(vg_codcasino, 1) Then
       indblo = True
    End If
    RS.Close: Set RS = Nothing
    ...
    RS.Open RutinaLectura.Minutas(3, 0, 0, Val(vg_fecha), ""), vg_db, adOpenStatic
    If Not RS.EOF Then
       If Not ValidarMinutaPassword(vg_codcasino, vg_Clave_MVI) Then
...
'                       .TypeIntegerMax = 9999999
    ...
    RS.Open RutinaLectura.EstructuraFijaDia(1, vg_codregimen, vg_codservicio, Val(vg_fecha), "1"), vg_db, adOpenStatic
    If Not RS.EOF Then estfij = True
    RS.Close: Set RS = Nothing
    fecesf = 0
    If Not estfij Then
       RS1.Open RutinaLectura.MinutaFija(1, vg_codregimen, vg_codservicio, 0, 0, "", "", ""), vg_db, adOpenStatic
    ...
    For j = 2 To (.MaxCols - 1) Step 5
        ...
        If estfij Then
           ...
           RS.Open RutinaLectura.MinutaFijaDia(2, vg_codregimen, vg_codservicio, vg_fecha & Right("0" & inddia, 2), "1", ""), vg_db, adOpenStatic
    ...
    Dim vTotRac As Long, vCosVec As Double
    j = 2
    For i = 1 To UBound(VecCosenc)
        .Row = .MaxRows
        .Col = j + 2
...
Dim X As Long, numrac As Long, vCosVec As Double, vTotRac As Double
...
With vaSpread1
    For X = 1 To (.MaxRows - 1)
        ...
        If Trim(.text) <> "" And numrac > 0 Then
           .Col = Col + 2: VecCosenc((Int(Col / 5) + 1), 1) = Round(VecCosenc((Int(Col / 5) + 1), 1) + (cosdia * numrac), vg_DCa)
        End If
    Next X
    .Row = .MaxRows
    .Col = Col + 1
...
Dim cosdia As Double, totdia As Double, totesf As Double, totrac As Double, sql1 As String, sql2 As String, sql3 As String
Dim Fecha As Long, xcol As Long, inddia As Long, fecesf As Double, nracre As Long, nracfo As Long
Dim aAp As String, estfij As Boolean
Dim RS As New ADODB.Recordset
DoEvents
...
estfij = False
'-------> Buscar datos estructura fija día
RS.Open RutinaLectura.MinutaFijaDia(1, vg_codregimen, vg_codservicio, Val(vg_fecha), "1", ""), vg_db, adOpenStatic
If Not RS.EOF Then estfij = True
RS.Close: Set RS = Nothing
fecesf = 0
If Not estfij Then
...
For j = 2 To (vaSpread1.MaxCols - 1) Step 5
    ...
    If Val(vaSpread1.text) > 0 Then
        If estfij Then
           '-------> Calcular datos desde tabla estructura fija día
           RS.Open RutinaLectura.MinutaFijaDia(2, vg_codregimen, vg_codservicio, Val(vg_fecha) & Right("0" & inddia, 2), "1", ""), vg_db, adOpenStatic
           DoEvents
