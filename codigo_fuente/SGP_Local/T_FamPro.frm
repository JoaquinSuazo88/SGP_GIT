...
Dim ivalidar As Integer, indlec As Integer
...
Set dest = nd
RS1.Open "SELECT * FROM a_tipopro WHERE tip_previo=0 AND tip_activo in ('1','S')", vg_db, adOpenStatic
If Not RS1.EOF Then
    Do While Not RS1.EOF
        Set rootnode = TvwDir.Nodes.Add(nd, tvwChild, "H" & RS1!tip_codigo, IIf(IsNull(RS1!tip_nombre), "", Trim(RS1!tip_nombre)))
        ' agregar un nodo hijo postizo, si fuera necesario
        If rootnode.Children = 0 Then
            RS2.Open "SELECT DISTINCT tip_codigo FROM a_tipopro WHERE tip_previo=" & RS1!tip_codigo & " AND tip_activo in ('1','S')", vg_db, adOpenStatic
            If Not RS2.EOF Then
    ...
    Loop
Else
   Gl_Ac_Botones Me, 1, IIf(vg_modprod = True, 2, 6), modo
End If
...
If modo = "A" Or modo = "M" Then
    ...
    If modo = "A" Then
        vg_db.BeginTrans
        RS1.Open "SELECT tip_codigo FROM a_tipopro WHERE tip_activo in ('1','S') ORDER BY tip_codigo DESC", vg_db, adOpenStatic
        indlec = 1
        If Not RS1.EOF Then indice = RS1!tip_codigo + 1 Else indice = 1
        RS1.Close: Set RS1 = Nothing
        indlec = 0
        vg_db.Execute "INSERT INTO a_tipopro VALUES (" & indice & ", '" & Mid(LimpiaDato(Trim(TvwDir.Nodes(dest.Index).text)), 1, 35) & "', " & codpadre & ")"
        vg_db.CommitTrans
        TvwDir.Nodes(dest.Index).Key = "H" & indice
        dest.EnsureVisible
    ElseIf modo = "M" Then
        vg_db.BeginTrans
        vg_db.Execute "UPDATE a_tipopro SET tip_nombre='" & Mid(LimpiaDato(Trim(TvwDir.Nodes(dest.Index).text)), 1, 35) & "' WHERE tip_codigo=" & Val(Mid(TvwDir.Nodes(dest.Index).Key, 2))
        vg_db.CommitTrans
    End If
    modo = "M"
...
ElseIf ivalidar = 0 Then
    If modo = "M" Then
       RS1.Open "SELECT * FROM a_tipopro WHERE tip_codigo=" & codpadre, vg_db, adOpenStatic
       If Not RS1.EOF Then codigo = RS1!tip_codigo
       RS1.Close: Set RS1 = Nothing
    Else
       codigo = codpadre
    End If
...
If TvwDir.SelectedItem.Index = 1 Then Exit Sub
Set dest = TvwDir.SelectedItem
RS1.Open "SELECT DISTINCT tip_codigo FROM a_tipopro WHERE tip_codigo=" & Val(Mid(TvwDir.Nodes(dest.Index).Key, 2, 20)), vg_db, adOpenStatic
sw = RS1("tip_codigo")
If Not RS1.EOF Then
    RS2.Open "SELECT * FROM a_tipopro WHERE tip_previo=" & sw, vg_db, adOpenStatic
    If Not RS2.EOF Then
...
vg_db.BeginTrans
vg_db.Execute "DELETE a_tipopro from a_tipopro WHERE tip_codigo = " & Val(Mid(TvwDir.Nodes(dest.Index).Key, 2, 20)) & ""
...
If modo = "A" Then
   ...
   modo = "M"
Else
   RS1.Open "SELECT tip_nombre FROM a_tipopro WHERE tip_codigo=" & Val(Mid(TvwDir.Nodes(dest.Index).Key, 2, 20)) & " AND tip_activo in ('1','S')", vg_db, adOpenStatic
   If Not RS1.EOF Then TvwDir.Nodes(dest.Index).text = IIf(IsNull(RS1!tip_nombre), "", Trim(RS1!tip_nombre))
   RS1.Close: Set RS1 = Nothing
   TvwDir.SetFocus
End If
If dest Is Nothing Then Gl_Ac_Botones Me, 1, 2, modo Else Gl_Ac_Botones Me, 1, 1, modo
...
RS1.Open "SELECT * FROM a_tipopro WHERE tip_previo=" & Val(Mid(TvwDir.Nodes(dest1.Index).Key, 2, 20)) & " AND tip_activo in ('1','S')", vg_db, adOpenStatic
If Not RS1.EOF Then
   Do While Not RS1.EOF
      Set nd = TvwDir.Nodes.Add(dest1.Index, tvwChild, "H" & RS1!tip_codigo, IIf(IsNull(RS1!tip_nombre), "", RS1!tip_nombre))
      If nd.Children = 0 Then
         RS2.Open "SELECT DISTINCT tip_codigo FROM a_tipopro WHERE tip_previo=" & RS1!tip_codigo & " AND tip_activo in ('1','S')", vg_db, adOpenStatic
         ' la propiedad Texto de los nodos positivos es "***"
         If Not RS2.EOF Then TvwDir.Nodes.Add nd.Index, tvwChild, , "*"
         RS2.Close: Set RS2 = Nothing
      End If
      RS1.MoveNext
   Loop
