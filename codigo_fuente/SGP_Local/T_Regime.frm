...
With vaSpread1
    ...
    If modo = "A" Then
        vg_db.BeginTrans
        RS1.Open RutinaLectura.Regimen(3, 0, ""), vg_db, adOpenStatic
        If Not RS1.EOF Then
            RS1.MoveFirst
            codigo = RS1!reg_codigo + 1
            If codigo > 9999 Then
               vg_db.RollbackTrans
               .Row = Fila
               .DeleteRows vaSpread1.Row, 1
               .MaxRows = .MaxRows - 1
        ...
        End If
        RS1.Close: Set RS1 = Nothing
        vg_db.Execute "INSERT INTO a_regimen (reg_codigo, reg_nombre) VALUES (" & codigo & ", '" & Trim(Nombre) & "')"
        vg_db.CommitTrans
        .Col = 1: .Value = codigo
    Else
        vg_db.BeginTrans
        vg_db.Execute "UPDATE a_regimen SET reg_nombre='" & Trim(Nombre) & "' WHERE reg_codigo=" & codigo
        vg_db.CommitTrans
    End If
...
If Combo1.ItemData(Combo1.ListIndex) = 0 Then
    RS2.Open RutinaLectura.Regimen(4, 0, UCase(LimpiaDato(fpText1.text))), vg_db, adOpenStatic
ElseIf Combo1.ItemData(Combo1.ListIndex) = 1 Then
    RS2.Open RutinaLectura.Regimen(5, 0, UCase(LimpiaDato(fpText1.text))), vg_db, adOpenStatic
End If
With vaSpread1
    ...
    If Not RS2.EOF Then
       Do While Not RS2.EOF
          ...
          .Value = RS2!reg_codigo
          .Col = 2
          .CellType = IIf(RS2!reg_codigo > 9999, CellTypeStaticText, CellTypeEdit)
          .Value = IIf(IsNull(RS2!reg_nombre), "", Trim(RS2!reg_nombre))
          RS2.MoveNext: i = i + 1
       Loop
       Gl_Ac_Botones Me, 1, 1, modo
    End If
...
With vaSpread1
    .MaxRows = 0
    RS2.Open RutinaLectura.Regimen(1, 0, ""), vg_db, adOpenStatic
    Do While Not RS2.EOF
        .MaxRows = .MaxRows + 1
        .Row = .MaxRows
        .Col = 1: .Value = RS2!reg_codigo
        .Col = 2
        .CellType = IIf(RS2!reg_codigo > 9999, CellTypeStaticText, CellTypeEdit)
        .Value = IIf(IsNull(RS2!reg_nombre), "", Trim(RS2!reg_nombre))
        RS2.MoveNext
...
With vaSpread1
    .Row = .ActiveRow
    .Col = 1: codigo = Val(.Value)
    RS1.Open RutinaLectura.Regimen(2, codigo, ""), vg_db, adOpenStatic
    If Not RS1.EOF Then
       .Col = 2: .Value = IIf(IsNull(RS1!reg_nombre), "", Trim(RS1!reg_nombre))
    End If
    RS1.Close: Set RS1 = Nothing
End With
OpGr = False
