...
Begin VB.Form T_EnvSap 
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.ComboBox Combo1 
         Appearance      =   0  'Flat
         Height          =   315
         ItemData        =   "T_EnvSap.frx":0000
         Left            =   2010
         List            =   "T_EnvSap.frx":000A
   ...
   Begin FPSpread.vaSpread vaSpread1 
      ...
      MaxCols         =   2
      ProcessTab      =   -1  'True
      SpreadDesigner  =   "T_EnvSap.frx":001E
      ScrollBarTrack  =   3
      ClipboardOptions=   0
   End
...
If modo = "A" Then
   RS1.Open "SELECT tii_codigo FROM a_tipointerfaz ORDER BY tii_codigo DESC", vg_db, adOpenStatic
   If Not RS1.EOF Then
       RS1.MoveFirst
       codigo = RS1!tii_codigo + 1
   Else
       codigo = 1
   End If
   RS1.Close: Set RS1 = Nothing
   vg_db.BeginTrans
   vg_db.Execute "INSERT INTO a_tipointerfaz (tii_codigo, tii_nombre) VALUES (" & codigo & ", '" & Trim(Nombre) & "')"
   vg_db.CommitTrans
   vaSpread1.Col = 1: vaSpread1.Lock = True: vaSpread1.text = codigo
Else
    vg_db.BeginTrans
    vg_db.Execute "UPDATE a_tipointerfaz SET tii_nombre='" & Trim(Nombre) & "' WHERE tii_codigo=" & codigo & ""
    vg_db.CommitTrans
End If
...
If Combo1.ItemData(Combo1.ListIndex) = 0 Then
    RS2.Open "SELECT tii_codigo, tii_nombre FROM a_tipointerfaz WHERE tii_codigo LIKE '%" & UCase(LimpiaDato(fpText1.text)) & "%'", vg_db, adOpenStatic
ElseIf Combo1.ItemData(Combo1.ListIndex) = 1 Then
    RS2.Open "SELECT tii_codigo, tii_nombre FROM a_tipointerfaz WHERE UPPER(tii_nombre) LIKE '%" & UCase(LimpiaDato(fpText1.text)) & "%'", vg_db, adOpenStatic
End If
...
If Not RS2.EOF Then
   Do While Not RS2.EOF
      vaSpread1.Row = i
      i = i + 1
      vaSpread1.Col = 1: vaSpread1.Lock = True: vaSpread1.TypeHAlign = TypeHAlignLeft: vaSpread1.text = RS2!tii_codigo
      vaSpread1.Col = 2: vaSpread1.Lock = False: vaSpread1.TypeHAlign = TypeHAlignLeft: vaSpread1.text = IIf(IsNull(RS2!tii_nombre), "", Trim(RS2!tii_nombre))
      RS2.MoveNext
   Loop
   Gl_Ac_Botones Me, 1, 1, modo
End If
...
vaSpread1.Visible = False
vaSpread1.MaxRows = 0
RS2.Open "SELECT tii_codigo, tii_nombre FROM a_tipointerfaz ORDER BY tii_codigo", vg_db, adOpenStatic
Do While Not RS2.EOF
    vaSpread1.MaxRows = vaSpread1.MaxRows + 1
    vaSpread1.Row = vaSpread1.MaxRows
    vaSpread1.Col = 1: vaSpread1.Lock = True: vaSpread1.text = IIf(IsNull(RS2!tii_codigo), "", RS2!tii_codigo)
    vaSpread1.Col = 2: vaSpread1.Lock = False: vaSpread1.text = IIf(IsNull(RS2!tii_nombre), "", Trim(RS2!tii_nombre))
    RS2.MoveNext
Loop
RS2.Close: Set RS2 = Nothing
Gl_Ac_Botones Me, 1, 1, modo
...
vaSpread1.Row = vaSpread1.ActiveRow
vaSpread1.Col = 1: codigo = Val(vaSpread1.text)
RS1.Open "SELECT * FROM a_tipointerfaz WHERE tii_codigo=" & codigo & "", vg_db, adOpenStatic
If Not RS1.EOF Then
   vaSpread1.Col = 2: vaSpread1.Value = IIf(IsNull(RS1!tii_nombre), "", Trim(RS1!tii_nombre))
End If
RS1.Close: Set RS1 = Nothing
OpGr = False
End Sub
