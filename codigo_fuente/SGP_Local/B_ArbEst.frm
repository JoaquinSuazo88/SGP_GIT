...
fg_carga ""
TvwDir.Nodes.Clear
RS1.Open "SELECT DISTINCT " & Suf & "codigo, " & Suf & "nombre FROM  " & Tabla & " WHERE " & Suf & "previo = 0", vg_db, adOpenStatic
If Not RS1.EOF Then
    Do While Not RS1.EOF
        ...
        If rootnode.Children = 0 Then
            RS2.Open "SELECT DISTINCT " & Suf & "codigo FROM  " & Tabla & " WHERE " & Suf & "previo = " & RS1(0) & "", vg_db, adOpenStatic
           If Not RS2.EOF Then
...
RS1.Open "SELECT * FROM  " & Tabla & " WHERE " & Suf & "previo = " & Val(Mid(TvwDir.Nodes(dest.Index).Key, 2, 20)) & " ORDER BY " & Suf & "nombre", vg_db, adLockReadOnly
If Not RS1.EOF Then
   Do While Not RS1.EOF
      Set nd = TvwDir.Nodes.Add(dest.Index, tvwChild, "H" & RS1(0), RS1(1), 4)
      dest.ExpandedImage = 4
      If nd.Children = 0 Then
         RS2.Open "SELECT DISTINCT " & Suf & "previo " & "FROM " & Tabla & " WHERE " & Suf & "previo = " & RS1(0) & "", vg_db, adOpenStatic
         If Not RS2.EOF Then
