...
Begin VB.Form T_CatDie 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Categoría Dietética"
   ClientHeight    =   4815
   ClientLeft      =   2835
   ClientTop       =   2505
   ClientWidth     =   6030
...
Dim ivalidar As Integer, CodPat As Integer, indlec As Integer
...
Case 15
    ...
    If dest Is Nothing Then MsgBox "No Existe Datos Imprimir", vbExclamation + vbOKOnly, "Categoría Dietética": Exit Sub
    I_CatDie
Case 18
    Me.Hide
    Unload Me
...
fg_carga ""
TvwDir.Nodes.Clear
Set nd = TvwDir.Nodes.Add(, , "R", "Categoría Dietética")
Set dest = nd
RS1.Open RutinaLectura.CategoriaDietetica(1, 0, ""), vg_db, adOpenStatic
If Not RS1.EOF Then
    Do While Not RS1.EOF
        Set rootnode = TvwDir.Nodes.Add(nd, tvwChild, "H" & RS1!car_codigo, IIf(IsNull(RS1!car_nombre), "", Trim(RS1!car_nombre)))
            ' agregar un nodo hijo postizo, si fuera necesario
        If rootnode.Children = 0 Then
           RS2.Open RutinaLectura.CategoriaDietetica(2, RS1!car_codigo, ""), vg_db, adOpenStatic
            If Not RS2.EOF Then
...
If modo = "A" Then
    vg_db.BeginTrans
    RS1.Open RutinaLectura.CategoriaDietetica(3, 0, ""), vg_db, adOpenStatic
    indlec = 1
    If Not RS1.EOF Then RS1.MoveFirst: indice = RS1!car_codigo + 1 Else indice = 1
    RS1.Close: Set RS1 = Nothing
    indlec = 0
    vg_db.Execute "INSERT INTO a_recetacatdie VALUES (" & indice & ", '" & Mid(LimpiaDato(Trim(TvwDir.Nodes(dest.Index).text)), 1, 50) & "'," & codpadre & ")"
    vg_db.CommitTrans
    TvwDir.Nodes(dest.Index).Key = "H" & indice
    dest.EnsureVisible
ElseIf modo = "M" Then
    vg_db.BeginTrans
    vg_db.Execute "UPDATE a_recetacatdie SET car_nombre='" & Mid(LimpiaDato(Trim(TvwDir.Nodes(dest.Index).text)), 1, 50) & "' WHERE car_codigo=" & Val(Mid(TvwDir.Nodes(dest.Index).Key, 2, 20)) & ""
    vg_db.CommitTrans
End If
modo = "M"
...
If ivalidar = 0 And LimpiaDato(Trim(TvwDir.Nodes(dest.Index).text)) = "" Then
    ivalidar = 1: MsgBox "Descripción Debe ser Informada", vbExclamation + vbOKOnly, "Categoría Dietética"
    Set TvwDir.SelectedItem = dest
    TvwDir.StartLabelEdit
    Exit Sub
ElseIf ivalidar = 0 Then
    If modo = "M" Then
        RS1.Open RutinaLectura.CategoriaDietetica(4, codpadre, ""), vg_db, adOpenStatic
        If Not RS1.EOF Then codigo = RS1!car_previo
        RS1.Close: Set RS1 = Nothing
    Else
        codigo = codpadre
    End If
    RS1.Open RutinaLectura.CategoriaDietetica(5, codigo, UCase(LimpiaDato(Trim(TvwDir.Nodes(dest.Index).text)))), vg_db, adOpenStatic
    If Not RS1.EOF Then
        If RS1!nreg = 1 And Nombre <> TvwDir.Nodes(dest.Index).text Then
            RS1.Close: Set RS1 = Nothing
            ivalidar = 1: MsgBox "Ya existe descripción", vbExclamation + vbOKOnly, "Categoría Dietética"
            Set TvwDir.SelectedItem = dest
            TvwDir.StartLabelEdit
            Exit Sub
        Else
...
If TvwDir.SelectedItem.Index = 1 Then Exit Sub
Set dest = TvwDir.SelectedItem
RS1.Open RutinaLectura.CategoriaDietetica(4, Val(Mid(TvwDir.Nodes(dest.Index).Key, 2, 20)), ""), vg_db, adOpenStatic
If Not RS1.EOF Then
    RS2.Open RutinaLectura.CategoriaDietetica(2, RS1!car_codigo, ""), vg_db, adOpenStatic
    If Not RS2.EOF Then
        MsgBox "No se puede Eliminar Dato, esta asociado a un Tipo Plato", vbCritical + vbOKOnly, "Categoría Dietética"
...
End If
RS1.Close: Set RS1 = Nothing
If MsgBox("Elimina registro...", vbQuestion + vbYesNo, Msgtitulo) = vbYes Then
   RS1.Open RutinaLectura.CategoriaDietetica(4, Val(Mid(TvwDir.Nodes(dest.Index).Key, 2, 20)), ""), vg_db, adOpenStatic
   If Not RS1.EOF Then CodPat = RS1!car_previo
   RS1.Close: Set RS1 = Nothing
   vg_db.BeginTrans
   vg_db.Execute "DELETE a_recetacatdie FROM a_recetacatdie WHERE car_codigo=" & Val(Mid(TvwDir.Nodes(dest.Index).Key, 2, 20))
...
Else
   RS1.Open RutinaLectura.CategoriaDietetica(4, Val(Mid(TvwDir.Nodes(dest.Index).Key, 2, 20)), ""), vg_db, adOpenStatic
   If Not RS1.EOF Then TvwDir.Nodes(dest.Index).text = Trim(RS1!car_nombre)
   RS1.Close: Set RS1 = Nothing
   TvwDir.SetFocus
End If
If dest Is Nothing Then Gl_Ac_Botones Me, 1, 2, modo Else Gl_Ac_Botones Me, 1, 1, modo
...
RS1.Open RutinaLectura.CategoriaDietetica(2, Val(Mid(TvwDir.Nodes(dest1.Index).Key, 2, 20)), ""), vg_db, adOpenStatic
If Not RS1.EOF Then
   Do While Not RS1.EOF
      Set nd = TvwDir.Nodes.Add(dest1.Index, tvwChild, "H" & RS1!car_codigo, RS1!car_nombre)
