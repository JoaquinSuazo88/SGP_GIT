...
Begin VB.Form P_CarRac 
   Caption         =   "Cargar Raciones Vigatec"
   ClientHeight    =   7170
   ClientLeft      =   1875
   ClientTop       =   2910
   ClientWidth     =   11580
   ...
   Begin FPSpread.vaSpread vaSpread1 
      ...
      MaxCols         =   11
      MaxRows         =   20
      SpreadDesigner  =   "P_CarRac.frx":030A
   End
End
Attribute VB_Name = "P_CarRac"
...
fg_centra Me
est = True
Msgtitulo = "Cargar Raciones Vigatec"
...
Do While Not EOF(1)
   ...
   If Trim(Mid(cpar, 130, 10)) = LimpiaDato(Trim(fpText(0).text)) Then
      ...
      Do While Not RS1.EOF
         vaSpread1.Col = 9: lisnom = lisnom & IIf(lisnom <> "", Chr$(9), "") & Trim(RS1!reg_nombre)
         vaSpread1.Col = 8: liscod = liscod & IIf(liscod <> "", Chr$(9), "") & RS1!reg_codigo
         vaSpread1.Col = 9: vaSpread1.TypeComboBoxList = lisnom
         vaSpread1.Col = 8: vaSpread1.TypeComboBoxList = liscod
         RS1.MoveNext
      Loop
      RS1.MoveFirst
      '------- agregar servicio
      ...
      Do While Not RS2.EOF
         vaSpread1.Col = 11: lisnom = lisnom & IIf(lisnom <> "", Chr$(9), "") & Trim(RS2!ser_nombre)
         vaSpread1.Col = 10: liscod = liscod & IIf(liscod <> "", Chr$(9), "") & RS2!ser_codigo
         vaSpread1.Col = 11: vaSpread1.TypeComboBoxList = lisnom
         vaSpread1.Col = 10: vaSpread1.TypeComboBoxList = liscod
         RS2.MoveNext
      Loop
     RS2.MoveFirst
   End If
...
Case 1
    ...
    For i = 1 To vaSpread1.MaxRows
        ...
        vaSpread1.Col = 8: codreg = Val(vaSpread1.text)
        vaSpread1.Col = 10: codser = Val(vaSpread1.text)
        '------- Borrar y Grabar alumnos raciones
        vg_db.Execute "DELETE FROM b_alumnosraciones WHERE alr_cencos='" & LimpiaDato(Trim(fpText(0).text)) & "' AND alr_rutalumno='" & rutalu & "' AND alr_fecha=" & Fecha & ""
        vg_db.Execute "INSERT INTO b_alumnosraciones VALUES ('" & LimpiaDato(Trim(fpText(0).text)) & "', '" & rutalu & "', " & Fecha & ", '" & nomalu & "', '" & curso & "', '" & nomapo & "', '" & seralu & "', " & racion & ", " & codreg & ", " & codser & ")"
...
Case 9
    vaSpread1.Row = Row
    vaSpread1.Col = 9: indice = vaSpread1.TypeComboBoxCurSel
    vaSpread1.Col = 8: vaSpread1.TypeComboBoxCurSel = indice
'    vaSpread1.EditEnterAction = EditEnterActionNone
Case 11
    vaSpread1.Row = Row
    vaSpread1.Col = 11: indice = vaSpread1.TypeComboBoxCurSel
    vaSpread1.Col = 10: vaSpread1.TypeComboBoxCurSel = indice
'    vaSpread1.EditEnterAction = EditEnterActionNone
End Select
End Sub
