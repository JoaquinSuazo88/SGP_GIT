...
vaSpread1.MaxRows = 0
fg_FormatearCeldaGrillaNuemricaDecimal vaSpread1, 4
est = False
fg_descarga
End Sub
...
Case 1
    ...
    RS.Open "SELECT * FROM a_regimen WHERE reg_codigo=" & fpLongInteger1(1).Value & "", vg_db, adOpenStatic
    ...
    fpLongInteger1(1).Value = RS!reg_codigo: fpayuda(1).Caption = Trim(RS!reg_nombre)
    RS.Close: Set RS = Nothing
    RS.Open "SELECT * FROM a_servicio WHERE ser_codigo = " & fpLongInteger1(2).Value & "", vg_db, adOpenStatic
    If RS.EOF Then RS.Close: Set RS = Nothing: fpayuda(2).Caption = "": MsgBox "No existe servicio", vbExclamation + vbOKOnly, Msgtitulo: Exit Sub
    fpLongInteger1(2).Value = RS!ser_codigo: fpayuda(2).Caption = Trim(RS!ser_nombre)
    RS.Close: Set RS = Nothing
    isel = 0
