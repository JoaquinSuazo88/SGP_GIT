# Prompt — Documentación de Reportes SGP Local (Orientada a Lectura de Usuario)
...
NOMBRE_MD      = Venta_Articulo_Cafeteria.md
...
El sistema **SGP Local** gestiona producción y servicios de alimentación para casinos Sodexo Chile. Está desarrollado en **Visual Basic 6** con base de datos **SQL Server** (y compatibilidad Access legacy). Los formularios de tipo reporte (`I_`, `L_`, `R_`) permiten consultar, exportar e imprimir información operativa del casino.

Los documentos Markdown son de uso funcional: los leen analistas, jefes de casino y coordinadores que no saben programación. El documento debe estar ordenado según la secuencia lógica en que el usuario interactúa con la pantalla: primero entiende para qué sirve, luego qué necesita preparar, luego cómo opera, luego qué restricciones existen y finalmente qué obtiene. Los detalles técnicos van al final como referencia.
...
- **SPs o funciones SQL:** referencias a `.Execute`, llamadas con prefijo `sgp_`, `fg_Cal`, `RutinaLectura.`. Para formularios con múltiples tipos de informe, anota qué SP se llama en cada `Case`.
...
*Referencia de estilo: `{{RUTA_DOC}}\md_pantallas\SGP_Local\Lectura_Vales.md` — última actualización: 2026-03-24*
